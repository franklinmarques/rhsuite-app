                ...
                <div class="col-md-12">
                    ...
                    <ol class="breadcrumb" style="margin-bottom: 5px; background-color: #eee;">
                        <?php if ($nome_cargo): ?>
                            ...
                            <li>
                                <a href="<?= site_url('recrutamento_cargos/gerenciar/' . $recrutamento) ?>"><?= $nome_recrutamento ?></a>
                            </li>
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" id="btnSalvar_cargo" onclick="salvar_cargo()" class="btn btn-primary">
                                Salvar
                            </button>
                            ...
                            <form action="#" id="form" class="form-horizontal">
                                ...
                                <div class="form-body">
                                    ...
                                    <div class="row form-inline form-group">
                                        <label class="control-label col-md-3">Tempo máximo</label>
                                        <div class="col-md-9">
                                            <input type="number" class="form-control text-right" name="minutos_duracao"
                                                   style="width: 130px;"> minutos para a finalização do teste
                                        </div>
                                    </div>
                                        ...
                                        <div class="col-md-9 col-md-offset-3">
                                            <div class="checkbox">
                                                <label>
                                                    ...
                                                    Exibir perguntas em ordem aleatória
                                                </label>
                                            </div>
                                            <div class="checkbox">
                                                <label>
                                                    <input type="checkbox" name="alternativas" value="A">
                                                    Exibir alternativas em ordem aleatória
                                                </label>
                                            </div>
                                        </div>
                                    </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            table = $('#table').DataTable({
                ...
                "ajax": {
                    "url": "<?php echo site_url('recrutamento_processos/ajax_list/' . $id_usuario . '/' . ($nome_cargo ? $id_candidato : '')) ?>",
                    "type": "POST"
                },
                rowsGroup: [0, 1],
...
//                },
        ...
        function add_cargo() {
            $('#cargo').val(''); // reset form on modals
            $('.form-group').removeClass('has-error'); // clear error class
            $('.help-block').empty(); // clear error string
            $('#modal_cargos').modal('show'); // show bootstrap modal
            $('#modal_cargos .modal-title').text('Adicionar cargo/função'); // Set Title to Bootstrap modal title
            $('.combo_nivel1').hide();
        }
        ...
        function add_teste() {
            ...
            $('#form input[type="hidden"]:not([name="id_candidato"])').val(''); // reset hidden input form on modals
            $('.form-group').removeClass('has-error'); // clear error class
            $('.help-block').empty(); // clear error string
            $('#hora_inicio').val('00:00');
            $('#hora_termino').val('23:59');
            $('#modal_form').modal('show'); // show bootstrap modal
            $('#modal_form .modal-title').text('Adicionar processo'); // Set Title to Bootstrap modal title
            $('.combo_nivel1').hide();
        }
        ...
        function edit_teste(id) {
            ...
            $.ajax({
                url: "<?php echo site_url('recrutamento_processos/ajax_edit/') ?>/" + id,
                type: "POST",
                dataType: "JSON",
                success: function (data) {
                    ...
                    $('[name="minutos_duracao"]').val(data.minutos_duracao);
                    $('[name="perguntas"]').prop('disabled', data.aleatorizacao_ok !== 'P' && data.aleatorizacao_ok !== 'T');
                    $('[name="alternativas"]').prop('disabled', data.aleatorizacao_ok !== 'A' && data.aleatorizacao_ok !== 'T');
                    ...
                    $('#modal_form .modal-title').text('Editar processo'); // Set title to Bootstrap modal title
                },
        ...
        function save() {
            ...
            var url;
            if (save_method === 'add') {
                url = "<?php echo site_url('recrutamento_processos/ajax_add') ?>";
            } else {
                url = "<?php echo site_url('recrutamento_processos/ajax_update') ?>";
            }
        ...
        function salvar_cargo() {
            $('#btnSalvar_cargo').text('Salvando...'); //change button text
            $('#btnSalvar_cargo').attr('disabled', true); //set button disable

            // ajax adding data to database
            $.ajax({
                url: "<?php echo site_url('recrutamento_processos/ajax_addCargo') ?>",
                type: "POST",
                ...
                success: function (data) {
                    ...
                    $('#btnSalvar_cargo').text('Salvar'); //change button text
                    $('#btnSalvar_cargo').attr('disabled', false); //set button enable
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    alert('Error adding / update data');
                    $('#btnSalvar_cargo').text('Salvar'); //change button text
        ...
        function delete_teste(id) {
            if (confirm('Deseja remover?')) {
                ...
                $.ajax({
                    url: "<?php echo site_url('recrutamento_processos/ajax_delete') ?>/" + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function (data) {
