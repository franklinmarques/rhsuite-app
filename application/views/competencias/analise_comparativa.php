                ...
                <td class="text-right">
                    <?php if ($is_pdf == false): ?>
                        <a id="pdf" class="btn btn-sm btn-danger"
                           href="<?= site_url('competencias/relatorios/pdfAnalise_comparativa/' . $this->uri->rsegment(3)); ?>"
                                    ...
                                    <?php foreach ($dimensao->colaboradores as $colaborador): ?>
                                        <tr class='text-right'>
                                            ...
                                            <?php case $colaborador->nivel > $dimensao->nivel: ?>
                                                <td class="text-success"><strong><?= $colaborador->nivel ?></strong>
                                                </td>
                                                <td class="text-success"><strong><?= $colaborador->gapNivel ?></strong>
                                                </td>
                                                <?php break; ?>
                                            <?php case $colaborador->nivel < $dimensao->nivel: ?>
                                                <td class="text-danger"><strong><?= $colaborador->nivel ?></strong></td>
                                                <td class="text-danger"><strong><?= $colaborador->gapNivel ?></strong>
                                                </td>
                                                <?php break; ?>
                                            <?php default: ?>
                                            ...
                                            <?php case $colaborador->atitude > $dimensao->atitude: ?>
                                                <td class="text-success"><strong><?= $colaborador->atitude ?></strong>
                                                </td>
                                                <td class="text-success">
                                                    <strong><?= $colaborador->gapAtitude ?></strong></td>
                                                <?php break; ?>
                                            <?php case $colaborador->atitude < $dimensao->atitude: ?>
                                                <td class="text-danger"><strong><?= $colaborador->atitude ?></strong>
                                                </td>
                                            ...
                                            <?php case $colaborador->IDc > $dimensao->IDc: ?>
                                                ...
                                                <td class="text-success"><strong><?= $colaborador->gapIDc ?></strong>
                                                </td>
                                                <?php break; ?>
                                            <?php case $colaborador->IDc < $dimensao->IDc: ?>
                                                <td class="text-danger"><strong><?= $colaborador->IDc ?></strong></td>
                                                <td class="text-danger"><strong><?= $colaborador->gapIDc ?></strong>
                                                </td>
                                                <?php break; ?>
                                            <?php default: ?>
...
<script>
    ...
    function gerar_comparativo() {
        ...
        $.ajax({
            ...
            'data': form,
            'beforeSend': function () {
                $('#gerar_comparativo').text('Gerando comparativo...').attr('disabled', true);
            },
            'success': function (json) {
                $('html, body').animate({'scrollTop': 600}, 'swing');
                if (json.length > 0) {
