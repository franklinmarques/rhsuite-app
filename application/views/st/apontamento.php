...
<section id="main-content">
	<section class="wrapper">
						...
						<div class="btn-group">
							...
							<ul class="dropdown-menu">
								<?php if ($this->session->userdata('nivel') != 11): ?>
									<li><a href="<?= site_url('st/colaboradores'); ?>"><i
												class="glyphicon glyphicon-list text-primary"></i> Colaboradores</a>
									</li>
								<?php endif; ?>
								<?php if ($modo_privilegiado): ?>
									<li><a href="<?= site_url('st/contratos'); ?>"><i
												class="glyphicon glyphicon-list text-primary"></i> Contratos</a>
									</li>
									<li><a href="<?= site_url('st/postos'); ?>"><i
												class="glyphicon glyphicon-list text-primary"></i> Postos</a>
									</li>
									<li><a href="<?= site_url('st/detalhesEventos'); ?>"><i
												class="glyphicon glyphicon-list text-primary"></i> Detalhes de
											eventos</a></li>
								<?php endif; ?>
								<li><a href="<?= site_url('st/eventos'); ?>"><i
											class="glyphicon glyphicon-list text-primary"></i> Relatório de
										eventos</a></li>
							</ul>
						</div>
					</div>
					...
					<div class="panel-body">
						...
						<ul class="nav nav-tabs" role="tablist">
							<li role="presentation" style="font-size: 14px; font-weight: bolder" class="active"><a
									...
									data-toggle="tab">Apontamento mensal</a></li>
							<?php if ($modo_privilegiado): ?>
								<li role="presentation" style="font-size: 14px; font-weight: bolder"
									class="aba_totalizacao"><a
							...
							<li role="presentation" style="font-size: 14px; font-weight: bolder; display: none;"
								class="emtu"><a
									...
									role="tab" data-toggle="tab">Medição consolidada</a></li>
							<li role="presentation" style="font-size: 14px; font-weight: bolder; display: none;"
								class="emtu"><a
						...
						<div class="tab-content" style="border: 1px solid #ddd; border-top-width: 0;">
								...
								<div role="tabpanel" class="tab-pane" id="totalizacao_consolidada">
									<table id="table_totalizacao_consolidada"
										...
										<tr>
											...
											<th class="warning text-center"><span
													id="total_acrescido_consolidado">Total</span>
											</th>
										</tr>
										</thead>
										<tbody>
							...
							<div role="tabpanel" class="tab-pane" id="relatorio_producao_emtu">
								...
								<table id="table_producao_emtu" class="table table-hover table-condensed table-bordered"
									   cellspacing="0" width="100%">
									<thead>
									<tr>
										<th rowspan="2" class="warning" style="vertical-align: middle;">
						...
						<form action="#" id="busca" class="form-horizontal" autocomplete="off">
							...
							<div class="row">
								...
								<div class="col-md-4">
									...
									<?php echo form_dropdown('funcao', $funcao, '', 'class="form-control input-sm"'); ?>
								</div>
								<div class="col-md-2">
		...
		<div class="modal fade" id="modal_feriado" role="dialog">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
					...
					<div class="modal-body">
						<form action="#" id="form_feriado" class="form-horizontal" autocomplete="off">
							...
							<div class="row form-group">
								...
								<div class="col-md-6">
									<p id="data_feriado" class="form-control-static"></p>
								</div>
								<div class="col-md-4 text-right">
									<button type="button" id="btnSaveFeriado" onclick="salvar_feriado()"
											class="btn btn-success">
										Salvar
									</button>
									<button type="button" id="btnLimparFeriado" onclick="limpar_feriado()"
							...
							<div class="row form-group">
								...
								<div class="col-md-9">
									<label class="checkbox-inline">
										<input type="radio" name="status" value="FR"> Feriado
									</label>
									<label class="checkbox-inline">
							...
							<hr>
							<div class="row form-group">
								<label class="control-label col-md-3">Qtde. novos processo</label>
								<div class="col-md-2">
							...
							<div class="row form-group">
								<label class="control-label col-md-3">Qtde. analistas</label>
								<div class="col-md-2">
									<input type="text" name="qtde_analistas" class="form-control numero">
								</div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        if ('<?= $modo_privilegiado ?>') {
            table_totalizacao = $('#table_totalizacao').DataTable({
                ...
                'ajax': {
                    'url': '<?php echo site_url('st/apontamento/ajaxTotalizacao') ?>',
                    'type': 'POST',
                    'timeout': 90000,
                    'data': function (d) {
                        d.busca = busca;
                    ...
                    'dataSrc': function (json) {
                        ...
                        if (json.draw === 1) {
                            ...
                            $("#calculo").append('<div class="radio"><label><input type="radio" name="calculo_totalizacao" onclick="calcular_totalizacao()" value="1" checked> Cálculo por dia/hora</label></div> &emsp;');
                            $("#calculo").append('<div class="radio"><label><input type="radio" name="calculo_totalizacao" onclick="calcular_totalizacao()" value="2"> Cálculo por percentual</label></div>');
                            //$("#calculo").append('<div class="radio"><label><?php //echo form_radio('calculo_totalizacao', '1', true, 'onclick="calcular_totalizacao()"'); ?>// Cálculo por dia/hora</label></div> &emsp;');
                            //$("#calculo").append('<div class="radio"><label><?php //echo form_radio('calculo_totalizacao', '2', false, 'onclick="calcular_totalizacao()"'); ?>// Cálculo por percentual</label></div>');
                        }
                        return json.data;
                    }
                },
                ...
                'preDrawCallback': function () {
                    drawing_table_totalizacao = true;
                },
                'drawCallback': function () {
                    drawing_table_totalizacao = false;
                    set_edicao_evento();
                }
            });
        } else {
                ...
                'ajax': {
                    ...
                    'dataSrc': function (json) {
                        ...
                        if (json.draw === 1) {
                            ...
                            $("#calculo_consolidado").append('<div class="radio"><label><input type="radio" name="calculo_totalizacao_consolidado" onclick="calcular_totalizacao_consolidada()" value="1" checked> Cálculo por dia/hora</label></div> &emsp;');
                            $("#calculo_consolidado").append('<div class="radio"><label><input type="radio" name="calculo_totalizacao_consolidado" onclick="calcular_totalizacao_consolidada()" value="2"> Cálculo por percentual</label></div>');
                            //$("#calculo_consolidado").append('<div class="radio"><label><?php //echo form_radio('calculo_totalizacao_consolidado', '1', true, 'onclick="calcular_totalizacao_consolidada()"'); ?>// Cálculo por dia/hora</label></div> &emsp;');
                            //$("#calculo_consolidado").append('<div class="radio"><label><?php //echo form_radio('calculo_totalizacao_consolidado', '2', false, 'onclick="calcular_totalizacao_consolidada()"'); ?>// Cálculo por percentual</label></div>');
                        }
                        return json.data;
                    }
                },
        ...
        table_emtu = $('#table_emtu').DataTable({
            ...
            'ajax': {
                'url': '<?php echo site_url('st/apontamento/listarEMTU') ?>',
                'type': 'POST',
                'timeout': 90000,
                'data': function (d) {
                    d.busca = busca;
            ...
            'columnDefs': [
                ...
                {
                    'createdCell': function (td, cellData, rowData, row, col) {
                        if ($(table_emtu.column(col).header()).hasClass('text-danger')) {
                            $(td).css('background-color', '#e9e9e9');
                        }
                    },
                    'className': 'text-center',
        ...
        table_producao_emtu = $('#table_producao_emtu').DataTable({
            'info': false,
            'lengthChange': false,
            'searching': false,
            'paging': false,
            ...
            'ajax': {
                'url': '<?php echo site_url('st/apontamento/listarProducaoEMTU') ?>',
                'type': 'POST',
                'timeout': 90000,
                'data': function (d) {
                    d.busca = busca;
                ...
                'dataSrc': function (json) {
                    ...
                    for (i = 1; i <= 31; i++) {
                        if (i > 28) {
                            if (i > json.calendar.qtde_dias) {
                                table_producao_emtu.column(i + colunasUsuario).visible(false, false);
                                continue;
                            } else {
                                table_producao_emtu.column(i + colunasUsuario).visible(true, false);
                            }
                        }
                        var coluna = $(table_producao_emtu.columns(i + colunasUsuario).header());
                        coluna.text(json.calendar.dias[i]);
                        coluna.removeClass('text-danger').css('background-color', '');
                        if (json.calendar.semana[semana] === 'Sábado' || json.calendar.semana[semana] === 'Domingo') {
                            coluna.addClass('text-danger').css('background-color', '#dbdbdb');
            ...
            'columnDefs': [
                ...
                {
                    'createdCell': function (td, cellData, rowData, row, col) {
                        if ($(table_producao_emtu.column(col).header()).hasClass('text-danger')) {
                            $(td).css('background-color', '#e9e9e9');
                        }
                    },
                    'className': 'text-center',
    ...
    function edit_feriado(dia, i) {
        ...
        $.ajax({
            'url': '<?php echo site_url('st/apontamento/editarFeriado') ?>',
            'type': 'POST',
            'dataType': 'json',
            'data': {
                'busca': $('#busca').serialize(),
            ...
            'success': function (json) {
                ...
                $.each(json, function (key, value) {
                    if ($('#form_feriado [name="' + key + '"]').prop('type') === 'radio') {
                        $('#form_feriado [name="' + key + '"][value="' + value + '"]').prop('checked', value !== null);
                    } else {
                        $('#form_feriado [name="' + key + '"]').val(value);
                    }
                });

                if (json.id === undefined) {
                    $('#btnLimparFeriado').hide();
                } else {
                    $('#btnLimparFeriado').show();
                }

                $('#data_feriado').html(moment(dia, 'YYYY-MM-DD').format('DD/MM/YYYY'));
    ...
    function salvar_feriado() {
        $('#btnSaveFeriado').text('Salvando...');
        $('#btnSaveFeriado, #btnLimparFeriado').attr('disabled', true);
        $.ajax({
            'url': '<?php echo site_url('st/apontamento/salvarFeriado') ?>',
            'type': 'POST',
            'dataType': 'json',
            'data': $('#form_feriado').serialize(),
            'success': function (json) {
                if (json.erro) {
                    alert(json.erro);
                    return false;
                ...
                $('#btnSaveFeriado').text('Salvar');
                $('#btnSaveFeriado, #btnLimparFeriado').attr('disabled', false);
                reload_table();
            },
            'error': function (jqXHR, textStatus, errorThrown) {
                alert('Error get data from ajax');
    ...
    function limpar_feriado() {
        if (confirm('Deseja limpar os eventos desta data?')) {
            $('#btnLimparFeriado').text('Limpando...').attr('disabled', true);
            $('#btnSaveFeriado, #btnLimparFeriado').attr('disabled', true);
            $.ajax({
                'url': '<?php echo site_url('st/apontamento/limparFeriado') ?>',
                'type': 'POST',
                'dataType': 'json',
                'data': {'id': $('#form_feriado [name="id"]').val()},
                'success': function (json) {
                    if (json.erro) {
                        alert(json.erro);
                        return false;
    ...
    function imprimir_medicao_consolidada() {
        ...
        window.open('<?php echo site_url('st/apontamento/imprimirMedicaoConsolidada'); ?>/q?' + $('#busca').serialize() + '&dia_fechamento=' + $('#dia_fechamento').val() + '&consolidado=1', '_blank');
    }

    function imprimir_relatorio_producao() {
        if ($('#busca [name="depto"]').val() === '' || $('#busca [name="area"]').val() === '' || $('#busca [name="setor"]').val() === '') {
            alert('Para gerar o relatório, ajuste os filtros de Departamento, Área e Setor.');
            return false;
        }
        window.open('<?php echo site_url('st/apontamento/imprimirRelatorioProducao'); ?>/q?' + $('#busca').serialize() + '&dia_fechamento=' + $('#dia_fechamento').val() + '&consolidado=1', '_blank');
    }
