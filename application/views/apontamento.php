						...
						<div class="btn-group">
							...
							<ul class="dropdown-menu">
								<?php if ($this->session->userdata('nivel') != 11): ?>
									<li><a href="apontamento_colaboradores"><i
...
<script>
    ...
    $(document).ready(function () {
        ...
        table_producao_emtu = $('#table_producao_emtu').DataTable({
            ...
            'ajax': {
                'url': '<?php echo site_url('apontamento/listarProducaoEMTU') ?>',
                'type': 'POST',
                'timeout': 90000,
                'data': function (d) {
                    d.busca = busca;
    ...
    function edit_feriado(dia) {
        ...
        $.ajax({
            'url': '<?php echo site_url('apontamento/editarFeriado') ?>',
            'type': 'POST',
            'dataType': 'json',
            'data': {
                'busca': $('#busca').serialize(),
    ...
    function salvar_feriado() {
        ...
        $.ajax({
            'url': '<?php echo site_url('apontamento/salvarFeriado') ?>',
            'type': 'POST',
            'dataType': 'json',
    ...
    function limpar_feriado() {
        if (confirm('Deseja limpar os eventos desta data?')) {
            ...
            $('#btnSaveFeriado, #btnLimparFeriado').attr('disabled', true);
            $.ajax({
                'url': '<?php echo site_url('apontamento/limparFeriado') ?>',
                'type': 'POST',
                'dataType': 'json',
    ...
    function imprimir_medicao_consolidada() {
        ...
        window.open('<?php echo site_url('apontamento/imprimirMedicaoConsolidada'); ?>/q?' + $('#busca').serialize() + '&dia_fechamento=' + $('#dia_fechamento').val() + '&consolidado=1', '_blank');
    }
