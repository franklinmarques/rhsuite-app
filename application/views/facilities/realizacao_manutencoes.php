...
<head>
    ...
    <title>CORPORATE RH - LMS - Formulário de Realização de Manutenções</title>
...
<div class="container-fluid">
    ...
    <htmlpageheader name="myHeader">
        <table id="table" class="table table-condensed" style="margin-bottom: 5px;">
            ...
            <tr style='border-top: 5px solid #ddd;'>
                <th colspan="3" style="text-align: center;">
                    <h3 class="text-center" style="font-weight: bold;">PROGRAMA DE MANUTENÇÃO PERIÓDICA</h3>
                </th>
            </tr>
            </thead>
        </table>
        <?php if ($is_pdf == false): ?>
            <div class="row">
                <div class="col col-md-6">
                    <h5><strong>Identificação do plano:</strong> <?= $nomeManutencao; ?></h5>
                </div>
                <div class="col col-md-6">
                    <h5><strong>Mês/ano da manutenção:</strong> <?= date('m/Y'); ?></h5>
                </div>
            </div>
            <div class="row">
                <div class="col col-md-6">
        ...
        <?php else: ?>
            <p>
            <h5><span style="font-weight: bold;">Identificação do plano:</span> <?= $nomeManutencao; ?></h5>
            <h5><span style="font-weight: bold;">Mês/ano da manutenção:</span> <?= date('m/Y'); ?></h5>
    ...
    <?php if (empty($manutencoes)): ?>
        <table id="no_itens" class="table table-bordered table-condensed">
            ...
            <tr class="active">
                ...
                <th rowspan="2" class="text-center">Manutenção realizada</th>
    ...
    <?php foreach ($manutencoes as $manutencao): ?>
        <table class="table table-bordered table-condensed itens">
                ...
                <th colspan="10">
                    <h4>
                        <strong>Unidade:</strong> <span
                                style="font-weight: normal;"><?= $manutencao['nome']->unidade; ?></span>&emsp;
                        <strong>Andar:</strong> <span
                                style="font-weight: normal;"><?= $manutencao['nome']->andar; ?></span>&emsp;
                        <strong>Sala:</strong> <span
                                style="font-weight: normal;"><?= $manutencao['nome']->sala; ?></span>
                    </h4>
                </th>
            </tr>
            <tr class="active">
            ...
            <tbody>
            <?php echo form_open('facilities/manutencoes/salvarItens/' . $this->uri->rsegment(3), 'data-aviso="alert" class="ajax-upload" id="form" autocomplete="off"'); ?>
            <?php foreach ($manutencao['subitens'] as $subitem): ?>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body form">
                    ...
                    <form action="#" id="form_os" class="form-horizontal" autocomplete="off">
                        ...
                        <input type="hidden" value="" name="id_modelo_manutencao">
...
<script>
    ...
    function add_os(id_modelo_manutencao, numero_os) {
        $('.form-group').removeClass('has-error'); // clear error class
        $('.help-block').empty(); // clear error string
        if (numero_os === undefined) {
            numero_os = '<?= $novaOS; ?>';
        }
        $('[name="id_modelo_manutencao"]').val(id_modelo_manutencao);
    ...
    function salvar_os() {
        ...
        $.ajax({
            'url': '<?php echo site_url('facilities/Manutencoes/salvarOS') ?>',
            'type': 'POST',
            'dataType': 'json',
