...
<head>
    ...
    <title>CORPORATE RH - LMS - Gerenciar itens de Manuenção Periódica de Facilities</title>
...
<div class="container-fluid">
    ...
    <button class="btn btn-info" onclick="add_manutencao()"><i class="glyphicon glyphicon-plus"></i>
        Adicionar item de manutenção periódica de facilities
    </button>
    ...
    <?php if ($nomeItem): ?>
        <h5 class="text-primary">
            <strong>Facility: <?= $nomeItem ?></strong>
        </h5>
    <?php endif; ?>
    <br>
        ...
        <div class="modal-dialog" style="width: 612px;">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h3 class="modal-title">Cadastro de itens de manutenção periódica de facilities</h3>
                </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        table = $('#table').DataTable({
            ...
            'ajax': {
                'url': '<?php echo site_url('facilities/itensManutencao/ajaxList/' . $this->uri->rsegment(3)) ?>',
                'type': 'POST'
            },
            'columnDefs': [
                {
    ...
    function add_manutencao() {
        ...
        $('.modal-title').text('Cadastrar item de manutenção periódica de facilities');
    ...
    function edit_manutencao(id) {
        ...
        $.ajax({
            url: '<?php echo site_url('facilities/itensManutencao/ajaxEdit/') ?>',
            data: {
                id: id,
            ...
            success: function (json) {
                ...
                $('.modal-title').text('Editar item de manutenção periódica de facilities');
                $('#modal_form').modal('show');
            }
        });
    }
    ...
    function save() {
        ...
        var url;
        if (save_method === 'add') {
            url = '<?php echo site_url('facilities/itensManutencao/ajaxAdd') ?>';
        } else {
            url = '<?php echo site_url('facilities/itensManutencao/ajaxUpdate') ?>';
        }
    ...
    function delete_manutencao(id) {
        if (confirm('Deseja remover?')) {
            $.ajax({
                url: '<?php echo site_url('facilities/itensManutencao/ajaxDelete') ?>',
                data: {id: id},
