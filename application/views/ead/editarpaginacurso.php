            ...
            <div class="col-lg-12">
                <section class="panel">
                    <header class="panel-heading">
                        ...
                        <a class="btn btn-default btn-sm" href="<?php echo site_url('ead/pagina_curso/index/' . $row->id_curso); ?>" style="float: right; margin-top: -0.6%;">
                    ...
                    <div class="panel-body">
                        ...
                        <?php echo form_open_multipart('ead/pagina_curso/ajax_update/' . $row->id, 'data-aviso="alert" class="form-horizontal ajax-upload" autocomplete="off"'); ?>
                        ...
                        <div class="row">
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <label class="radio-inline">
                                            <input type="radio" name="modulo" autocomplete="off" value="ckeditor"<?= ($row->modulo == "ckeditor" ? ' checked="checked"' : ''); ?> />
                                            <i class="fa fa-file-zip-o"></i> Objetos (texto, figura, tabela, Flash, links)
                                        </label>
                                        <label class="radio-inline">
                                            <input type="radio" name="modulo" autocomplete="off" value="quiz"<?= ($row->modulo == "quiz" ? ' checked="checked"' : ''); ?> />
                                            <i class="fa fa-question-circle"></i> Quick quiz
                                        </label>
                                        <label class="radio-inline">
                                            <input type="radio" name="modulo" autocomplete="off" value="atividades"<?= ($row->modulo == "atividades" ? ' checked="checked"' : ''); ?> />
                                            <i class="fa fa-pencil"></i> Quiz atividades (avaliação)
                                        </label>
                                    </div>
                                    <div class="col-md-12">
                                        <label class="radio-inline">
                                            <input type="radio" name="modulo" autocomplete="off" value="pdf"<?= ($row->modulo == "pdf" ? ' checked="checked"' : ''); ?> />
                                            <i class="fa fa-file-word-o"></i> Arquivos (Word, PorwerPoint, Bloco de Notas, PDF)
                                        </label>
                                        <label class="radio-inline">
                                            <input type="radio" name="modulo" autocomplete="off" value="url"<?= ($row->modulo == "url" ? ' checked="checked"' : ''); ?> />
                            ...
                            <div class="col-md-3 text-right">
                                <p>
                                    ...
                                    <input id="gravacao_audio" name="gravacao_audio" type="hidden" value="<?= (!empty($row->gravacao_audio) ? $row->gravacao_audio : ''); ?>" readonly>
                        ...
                        <div class="row url" style="display: <?= (in_array($row->modulo, array('url')) ? 'block' : 'none'); ?>;">
                            <div class="form-group">
                                ...
                                <div class="col-sm-8 col-lg-9 controls">
                                    <div class="input-group">
                                        ...
                                        <input type="text" name="url" placeholder="Endereço da URL" value="<?= (!empty($row->url) ? $row->url : ''); ?>" class="form-control" <?= ($row->modulo == 'url' && empty($row->url) ? 'disabled=""' : ''); ?>/>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="row pdf url" style="display: <?= (in_array($row->modulo, array('pdf', 'url')) ? 'block' : 'none'); ?>;">
                            <div class="form-group">
                                <label class="col-sm-3 col-lg-2 control-label">Arquivo</label>
                                <div class="pdf col-sm-8 col-lg-9 controls" style="display: <?= (in_array($row->modulo, array('pdf')) ? 'block' : 'none'); ?>;">
                                    <div class="fileinput fileinput-new input-group" data-provides="fileinput">
                                        ...
                                        <span class="input-group-addon btn btn-default btn-file">
                                            ...
                                            <input type="file" name="pdf" accept=".pdf,.doc,.docx,.txt,.ppt,.pptx"/>
                                        </span>
                                        <a href="#" class="input-group-addon btn btn-default fileinput-exists" data-dismiss="fileinput">Remover</a>
                                    </div>
                                    <span class="help-inline"><?= ($row->pdf ? "<a href='" . base_url('arquivos/pdf/' . convert_accented_characters($row->pdf)) . "' target='_blank'>{$row->pdf}</a>" : ''); ?></span>
                                    <br>
                                    <i class="text-danger" style="font-size: larger;">Importante: O nome do arquivo não pode conter sinais especiais nem acentuação.</i>
                                </div>
                                <div class="url col-sm-8 col-lg-9 controls" style="display: <?= (in_array($row->modulo, array('url')) ? 'block' : 'none'); ?>;">
                                    <span class="help-inline"><?= ($row->arquivo_video ? "<a href='" . base_url('arquivos/videos/' . convert_accented_characters($row->arquivo_video)) . "' target='_blank'>{$row->arquivo_video}</a>" : ''); ?></span>
                                    <div class="fileinput fileinput-new input-group" data-provides="fileinput">
                                        <span class="input-group-addon">
                                            <input name="tipo_url" value="2" type="radio">
                                        </span>
                                        <div class="form-control <?= ($row->modulo == 'url' && $row->arquivo_video ? '' : 'disabled'); ?>" data-trigger="fileinput">
                                            <i class="glyphicon glyphicon-file fileinput-exists"></i>
                                            <span class="fileinput-filename"></span>
                                        </div>
                                        <span class="input-group-addon btn btn-default btn-file">
                                            <span class="fileinput-new">Selecionar arquivo</span>
                                            <span class="fileinput-exists">Alterar</span>
                                            <input type="file" name="arquivo_video" placeholder="Apenas vídeos .mp4 são suportados!" value="<?= ($row->arquivo_video ? $row->arquivo_video : ''); ?>" accept="video/mp4"/>
                                        </span>
                                        <a href="#" class="input-group-addon btn btn-default fileinput-exists" data-dismiss="fileinput">Remover</a>
                                    </div>
                        ...
                        <div class="row quiz atividades" style="display: <?= (in_array($row->modulo, array('quiz', 'atividades')) ? 'block' : 'none'); ?>;">
                            <input type="hidden" name="questoes_add" value="">
                            <input type="hidden" name="questoes_remove" value="">
                            <div class="col-sm-12">
                                <button type="button" class="btn btn-success" onclick="add_questao()"><i class="glyphicon glyphicon-plus"></i> Adicionar questao</button>
                                <a class="btn btn-success" href="<?= site_url('ead/biblioteca/') ?>"><i class="glyphicon glyphicon-list-alt"></i> Ver biblioteca de questões</a>
                            </div>
                        </div>
                        <div class="row quiz atividades" style="display: <?= (in_array($row->modulo, array('quiz', 'atividades')) ? 'block' : 'none'); ?>;">
                            <br>
                            <div class="form-group">
                                ...
                                <div class="col-sm-3 col-lg-2 controls">
                                    <input type="number" name="nota_corte" value="<?= $row->nota_corte ?>" min="0" max="100" step="1" class="form-control"/>
                                </div>
                                <div class="col-sm-6 col-lg-5">
                                    <div class="checkbox">
                                        <label>
                                            <input type="checkbox" name="questao_aleatoria"<?= in_array($row->aleatorizacao, array('P', 'T')) ? ' checked=""' : '' ?>/>
                                            Mostrar questões em ordem aleatória
                                        </label>
                                    </div>
                                    <div class="checkbox disabled">
                                        <label>
                                            <input type="checkbox" name="alternativa_aleatoria"<?= in_array($row->aleatorizacao, array('A', 'T')) ? ' checked=""' : '' ?>/>
                        ...
                        <div class="row quiz atividades" style="display: <?= (in_array($row->modulo, array('quiz', 'atividades')) ? 'block' : 'none'); ?>;">
                            <div class="form-group">
                                <label class="col-sm-3 col-lg-3 control-label">Resultado igual ou maior que NC</label>
                                <div class="col-sm-6 col-lg-5 form-inline">
                                    <div class="input-group">
                                        <span class="input-group-addon" id="sizing-addon2">Ir para página</span>
                                        <?php echo form_dropdown('id_pagina_aprovacao', $row->proxima_pagina, $row->id_pagina_aprovacao, 'autocomplete="off" class="form-control"'); ?>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="row quiz atividades" style="display: <?= (in_array($row->modulo, array('quiz', 'atividades')) ? 'block' : 'none'); ?>;">
                            <div class="form-group">
                                ...
                                <div class="col-sm-6 col-lg-5 form-inline">
                                    <div class="input-group">
                                        <span class="input-group-addon" id="sizing-addon2">Ir para página</span>
                                        <?php echo form_dropdown('id_pagina_reprovacao', $row->proxima_pagina, $row->id_pagina_reprovacao, 'autocomplete="off" class="form-control"'); ?>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row ckeditor url" style="display: <?= (in_array($row->modulo, array('ckeditor', 'url')) ? 'block' : 'none'); ?>;">
                            <div class="form-group">
                                <div class="col-sm-12 col-lg-12 controls">
                                    <textarea name="conteudo" id="conteudo" class="form-control" rows="3">
                                        <?= $row->conteudo; ?>
                                    </textarea>
                                </div>
                            </div>
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <audio id="audio" controls style="width: 100%;" src="<?= (!empty($row->audio) ? base_url("arquivos/media/$row->audio") : ''); ?>"></audio>
                                        <span class="help-inline text-primary"><?= $row->audio ?></span>
                                        <div class="checkbox">
                                            <label>
                                                <input type="checkbox" name="autoplay" value="1"<?= $row->autoplay ? ' checked=""' : '' ?>> Execução automática
                                            </label>
                                        </div>
                                        <hr>
                                        <div class="radio">
                                            <label>
                                                <input type="radio" name="audio_modo" id="audio_modo_2" value="2" checked=""> Enviar arquivo de áudio
                                            </label>
                                        </div>
                                        <div id="modo_2">
                                            <div class="fileinput fileinput-new input-group" data-provides="fileinput">
                                                ...
                                                <span class="input-group-addon btn btn-default btn-file">
                                                    <span class="fileinput-new">Selecionar arquivo</span>
                                                    <span class="fileinput-exists">Alterar</span>
                                                    <input type="file" name="arquivo_audio" placeholder="Apenas audio .mp3 são suportados!" accept="audio/mp3"/>
                                                </span>
                                                <a href="#" class="input-group-addon btn btn-default fileinput-exists" data-dismiss="fileinput">Remover</a>
                                            </div>
                                        </div>
                                        <div class="radio">
                                            <label>
                                                <input type="radio" name="audio_modo" id="audio_modo_1" value="1"> Gravar áudio usando microfone
                                            </label>
                                        </div>
                                        <div id="modo_1">
                                            <div id="time">
                                            ...
                                            <div class="text-center">
                                                <div id="buttons" class="btn-group" role="group">
                                                    <button type="button" id="record-audio" class="btn btn-default disabled" data-status="0">
                                                        <i class="glyphicon glyphicon-record text-danger"></i> Gravar
                                                    </button>
                                                    <button type="button" id="clear-audio" onclick="window.location.reload();" class="btn btn-default disabled" disabled>
                                                        <i class="glyphicon glyphicon-eject text-info"></i> Limpar
                                                    </button>
                                                    <button type="button" id="send-audio" class="btn btn-default disabled" disabled>
        ...
        <div class="modal fade" id="modal_questao" role="dialog">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                        ...
                        <form action="#" id="form_questao" class="form-horizontal">
                            ...
                            <div class="form-body">
                                <div class="row form-group">
                                    ...
                                    <div class="col-md-9">
                                        <input name="nome" placeholder="Digite o nome descritivo da questão" class="form-control" type="text">
                    ...
                    <div class="modal-footer">
                        <button type="button" id="btnSaveQuestao" onclick="save_questao()" class="btn btn-primary">Adicionar à lista</button>
                        ...
                        <form action="#" id="form_conteudo" class="form-horizontal">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <textarea name="conteudo" id="questao_conteudo" class="form-control" rows="16" placeholder="Insira o texto descritivo da questão aqui"></textarea>
                                </div>
                            </div>
                        </form>
                    </div>
                        ...
                        <form action="#" id="form_respostas" class="form-horizontal">
                            ...
                            <div class="form-body" style="padding-top: 0px;">
                                <div class="form-group" style="margin-bottom: 0px;">
                                    ...
                                    <div class="col-md-7">
                                        <h5 id="nome" style="overflow:hidden; text-overflow:ellipsis; white-space: nowrap; font-weight: bolder;"></h5>
                                    </div>
                                    <div class="col-md-3 text-right">
                                ...
                                <div class="text-right text-danger" style="margin-bottom: 20px;">
                                    <i><strong>* Peso igual a 1:</strong> verdadeiro / resposta correta. <strong>Peso igual a 0:</strong> falso / resposta incorreta.</i>
                                </div>
        ...
        <div class="modal fade" id="modal-video" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <video id="previewVideo" controls style="border: 1px solid #000; height: 240px; width: 100%;">
                            <source id="previewAudio" src="<?= (!empty($row->video) ? base_url("arquivos/media/$row->arquivo_video") : ''); ?>">
                        </video>
                        <hr/>
...
//    var editor;
        ...
        table = $('#table').DataTable({
            ...
            "ajax": {
                ...
                "data": function (d) {
                    ...
                    d.questoes_remove = JSON.stringify(questoes_remove);
                    return d;
                }
            },
    ...
    function edit_questao(id) {
        ...
        $.ajax({
            ...
            success: function (data)
            {
                ...
                $('#form_questao [name="criar_modelo"]').prop({checked: false, disabled: data.id_biblioteca !== null});
    ...
    function edit_questao_row(row)
    {
        ...
        $('#form_questao [name="observacoes"]').val(data.observacoes);
        $('#form_questao [name="id_biblioteca"]').val(data.id_biblioteca);
        $('#form_questao [name="criar_modelo"]').prop({checked: data.criar_modelo, disabled: data.id_biblioteca !== null});
        switch (data.aleatorizacao) {
            case 'P':
                $('#form_questao [name="perguntas"]').prop('checked', true);
                break;
    ...
    function edit_conteudo(id)
    {
        ...
        $.ajax({
            ...
            success: function (data)
            {
                $('#form_conteudo [name="id"]').val(data.id);
                $('#form_conteudo [name="conteudo"]').val(data.conteudo);
    ...
    function edit_respostas(id)
    {
        ...
        $.ajax({
            ...
            {
                $('#nome').text(data.nome);
                $('#form_respostas [name="id_questao"]').val(data.id_questao);
    ...
    {
        $('#btnSaveQuestao').text('Salvando...'); //change button text
        $('#btnSaveQuestao').attr('disabled', true); //set button disable
        ...
        delete questoes_remove[values.id];

        $('#modal_questao').modal('hide');
        reload_table();

        $('[name="questoes_add"]').val(JSON.stringify(questoes_add));
        $('[name="questoes_remove"]').val(JSON.stringify(questoes_remove));
