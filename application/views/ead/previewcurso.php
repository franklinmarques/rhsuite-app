                                ...
                                <div class="panel-body" style="overflow-y: auto; max-height: 600px; font-size: 12px;">
                                    <?php foreach ($paginas->result() as $k => $pagina): ?>
                                        <p style="margin: 0 0 5px; overflow:hidden; text-overflow:ellipsis; white-space: nowrap;<?= $pagina->id === $paginaatual->id ? 'background: #758FB0; color: #fff' : '' ?>">
                                            ...
                                            <?php endif; ?>
                                            <a href="<?php echo site_url('ead/cursos/preview/' . $pagina->curso . '/' . $pagina->ordem); ?>" title="<?= $pagina->titulo; ?>">
                                ...
                                <header class="panel-heading">
                                    ...
                                    <div style="float:right; margin-top: -0.5%;">
                                        ...
                                        <?php if ($this->uri->rsegment(4) > 0): ?>
                                            <a class="btn btn-primary btn-sm" href="<?php echo site_url('ead/cursos/preview/' . $this->uri->rsegment(3) . '/' . ((int) $this->uri->rsegment(4) - 1)); ?>">
                    ...
                    <div class="col-md-6">
                        <?php if ($this->uri->rsegment(4) > 0): ?>
                            <div class="box-content pull-left">
                                <p>
                                    <a class="btn btn-primary" href="<?php echo site_url('ead/cursos/preview/' . $this->uri->rsegment(3) . '/' . ((int) $this->uri->rsegment(4) - 1)); ?>">
                                        <i class="glyphicon glyphicon-arrow-left"></i> Anterior
                                    </a>
                                </p>
                            </div>
...
//                content: '<div class="alert alert-info clearfix"><audio id="player" controls="" src="<?= base_url('arquivos/media/' . $paginaatual->audio); ?>"></audio></div>'
            ...
            function finalizaPagina(curso, pagina) {
                if (curso > 0 && pagina > 0) {
                    var url = '<?php echo site_url('ead/treinamento/finalizar_pagina'); ?>/' + curso + '/' + pagina;
                    $.ajax({
                        url: url,
                        dataType: 'json',
                        success: function (data) {
                            if (data.total < 100) {
                                window.location.href = '<?= site_url('ead/cursos/preview/' . $this->uri->rsegment(3)); ?>/' + data.proxima;
                            }
                            if (data.total >= 100) {
