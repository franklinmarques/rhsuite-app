...
<script>
    ...
    $(function () {
        $('.enviaResposta').click(function () {
            ...
            var url = '<?= site_url('ead/treinamento_cliente/avaliar_atividade/'); ?>';
    ...
    function finalizaPagina(id_curso, id_pagina) {
        if (id_curso > 0 && id_pagina > 0) {
            var url = '<?php echo site_url('ead/treinamento_cliente/finalizar_pagina'); ?>';
            $.ajax({
                type: "POST",
                url: url,
                dataType: 'json',
                ...
                success: function (data) {
                    if (data.proxima !== null) {
                        window.location.href = '<?= site_url('ead/treinamento_cliente/acessar/' . $this->uri->rsegment(3)); ?>/' + data.proxima;
                    } else {
