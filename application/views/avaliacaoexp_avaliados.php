                        ...
                        <?php else: ?>
                            ...
                            <?php elseif ($tipo == '2'): ?>
                                <li><a href="<?= site_url('avaliacaoexp/periodo') ?>">Avaliação por Período de
                                        Experiência</a></li>
                            <?php else: ?>
                                <li><a href="<?= site_url('avaliacaoexp') ?>">Avaliação Periódica e Período de
                                        Experiência</a></li>
                            <?php endif; ?>
                        <?php endif; ?>
                                ...
                                <div class="form-body">
                                    ...
                                    <?php if ($id_avaliado): ?>
                                        ...
                                        <div class="row form-group">
                                            ...
                                            <div class="col-md-2">
                                                <input name="nota_corte" id="nota_corte" class="form-control"
                                                       type="number">
                                    ...
                                    <div id="avaliados" class="row form-group">
                                        <?php echo form_multiselect('id_avaliado[]', $colaboradoresBatch, array(), 'size="10" id="id_avaliados" class="demo2"') ?>
                                    </div>
                                </div>
                                <?php endif; ?>
                                <?php if ($tipo == '2'): ?>
...
?>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            //datepicker
            demo2 = $('#id_avaliados').bootstrapDualListbox({
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (id_avaliado) {
                        ...
                        $('#id_avaliados').html(data.avaliado).attr('multiple');
                        $('#id_avaliados option[value=""]').remove();
        ...
        function add_avaliado() {
            ...
            if (id_avaliado === true) {
                $('#form input[type="hidden"]:not([name="id_avaliado"], [name="id_avaliados"])').val(''); // reset hidden input form on modals
            } else {
                $('#form input[type="hidden"]:not([name="id_modelo"], [name="id_avaliacao"])').val(''); // reset hidden input form on modals
            }
        ...
        function add_grupo_avaliados() {
            ...
            demo2.bootstrapDualListbox('refresh', true);
            $('#modal_form').modal('show');
            $('.modal-title').text('Adicionar avaliados e avaliadores'); // Set title to Bootstrap modal title
            $('.combo_nivel1').hide();
        }
        ...
        function edit_avaliado(id) {
            save_method = 'update';
            $('#form')[0].reset(); // reset form on modals
            $('#form input[type="hidden"]:not([name="id_avaliacao"])').val(''); // reset hidden input form on modals
            $('.form-group').removeClass('has-error'); // clear error class
            $('.help-block').empty(); // clear error string
