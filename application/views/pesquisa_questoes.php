        ...
        <section class="wrapper">
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <button class="btn btn-info" onclick="add_categoria()"><i
                                        class="glyphicon glyphicon-plus"></i> Adicionar categoria
                            </button>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <button class="btn btn-info" onclick="editar_resposta()" style="margin:15px 0;"><i
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body form">
                            <form action="#" id="form_categoria" class="form-horizontal">
                                <input type="hidden" value="" name="id"/>
                                <input type="hidden" value="<?= $id_modelo ?>" name="id_modelo"/>
                                <div class="form-body">
                                    <div class="form-group">
                                        <label class="col-sm-3 col-lg-2 control-label">Nome categoria</label>
                                        <div class="col-sm-9 col-lg-10 controls">
                                            <input type="text" name="categoria"
                                                   placeholder="Digite aqui o nome da ategoria" value=""
                                                   class="form-control"/>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="modal-footer">
                            <button type="button" id="btnSave" onclick="salvar_categoria()" class="btn btn-success">
                                Salvar
                            </button>
                            ...
                            <form action="#" id="form" class="form-horizontal">
                                ...
                                <div class="form-body">
                                    <div class="row form-group">
                                        ...
                                        <div class="col-md-10">
                                            <?php echo form_dropdown('categoria', $categorias, '', 'class="form-control"'); ?>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            table = $('#table').DataTable({
                ...
                "ajax": {
                    "url": "<?php echo site_url('pesquisa_questoes/ajax_list/' . $id_modelo) ?>",
                    "type": "POST"
                },
                ...
                "drawCallback": function (settings) {
                    ...
                    api.column(0, {page: 'current'}).data().each(function (group, i) {
                        if (last !== group) {
                            ...
                            $(rows).eq(i).before(
                                '<tr class="group text-nowrap"><th>' + group + '</th><th>' + rowData[1] + '</th></tr>'
                            );
                            if (rowData[2] === null) {
                                $(rows).eq(i).before(
                                    '<tr class="dataTables_empty" valign="top">' +
                                    '<td colspan="2" class="text-muted">Nenhuma pergunta cadastrada</td>' +
                                    '</tr>').after().remove();
                            }
                            last = group;
                        }
            ...
            table2 = $('#table2').DataTable({
                ...
                // Load data for the table's content from an Ajax source
                "ajax": {
                    "url": "<?php echo site_url('pesquisa_questoes/ajax_listResposta/' . $id_modelo) ?>",
                    "type": "POST"
                },
                //Set column definition initialisation properties.
                "columnDefs": [
        ...
        function list_categorias() {
            $.ajax({
                url: "<?php echo site_url('pesquisa_questoes/ajax_listCategoria/') ?>",
                type: "GET",
                dataType: "HTML",
        ...
        function add_categoria() {
            save_method = 'add';
            $('#form_categoria')[0].reset(); // reset form on modals
            $('#form input[type="hidden"]:not([name="id_modelo"])').val(''); // reset hidden input form on modals
            $('.form-group').removeClass('has-error'); // clear error class
            $('.help-block').empty(); // clear error string

            $('#modal_categoria').modal('show'); // show bootstrap modal
            $('.modal-title').text('Adicionar categoria'); // Set Title to Bootstrap modal title
            $('.combo_nivel1').hide();
        }
        ...
        function edit_categoria(id) {
            ...
            $.ajax({
                url: "<?php echo site_url('pesquisa_questoes/ajax_categoria/') ?>/" + id,
                type: "GET",
                dataType: "JSON",
                success: function (data) {
                    $('#form_categoria [name="id"]').val(data.id);
                    $('#form_categoria [name="id_modelo"]').val(data.id_modelo);
                    $('#form_categoria [name="categoria"]').val(data.categoria);
        ...
        function edit_pergunta(id) {
            ...
            //Ajax Load data from ajax
            $.ajax({
                url: "<?php echo site_url('pesquisa_questoes/ajax_edit/') ?>/" + id,
                type: "GET",
                dataType: "JSON",
                success: function (data) {
                    $('[name="id"]').val(data.id);
        ...
        function editar_resposta() {
            ...
            //Ajax Load data from ajax
            $.ajax({
                url: "<?php echo site_url('pesquisa_questoes/ajax_editResposta/' . $id_modelo) ?>",
                type: "GET",
                dataType: "JSON",
                success: function (data) {
        ...
        function salvar_categoria() {
            ...
            if (save_method === 'add') {
                url = "<?php echo site_url('pesquisa_questoes/ajax_addCategoria') ?>";
            } else {
                url = "<?php echo site_url('pesquisa_questoes/ajax_updateCategoria') ?>";
            }
        ...
        function save() {
            ...
            if (save_method === 'add') {
                url = "<?php echo site_url('pesquisa_questoes/ajax_add') ?>";
            } else {
                url = "<?php echo site_url('pesquisa_questoes/ajax_update') ?>";
            }
        ...
        function salvar_resposta() {
            ...
            // ajax adding data to database
            $.ajax({
                url: "<?php echo site_url('pesquisa_questoes/ajax_updateResposta') ?>",
                type: "POST",
        ...
        function delete_pergunta(id) {
            if (confirm('Deseja remover?')) {
                ...
                $.ajax({
                    url: "<?php echo site_url('pesquisa_questoes/ajax_delete') ?>/" + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function (data) {
        ...
        function delete_categoria(id) {
            if (confirm('Deseja remover?\n\nAs perguntas relacionadas à categoria também serão removidas!')) {
                // ajax delete data to database
                $.ajax({
                    url: "<?php echo site_url('pesquisa_questoes/ajax_deleteCategoria') ?>/" + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function (data) {
                        //if success reload ajax table
