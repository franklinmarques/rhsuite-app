...
<section id="main-content">
	<section class="wrapper">
		...
		<!-- Bootstrap modal -->
		<div class="modal fade" id="modal_form_mae" role="dialog">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					<div class="modal-header">
						<div style="float: right;">
							<button type="button" id="btnSave" onclick="save_mae()" class="btn btn-success">Salvar
							</button>
					...
					<div class="modal-body form">
						<form action="#" id="form_mae" class="form-horizontal">
							<input type="hidden" value="" name="id"/>
							<input type="hidden" value="" name="id_mae"/>
							<div class="form-body">
								<div class="row form-group">
									<input type="hidden" name="tipo" value="" class="empresa">
									...
									<label class="control-label col-sm-2">Data limite</label>
									<div class="col-sm-2">
										<input name="data_limite" placeholder="dd/mm/aaaa"
											   class="form-control text-center date empresa" type="text">
										<span class="help-block"></span>
									</div>
									<label class="control-label col-sm-1">Prioridade</label>
									<div class="col-sm-2">
									...
									<label class="control-label col-sm-2">Lembrar dias antes</label>
									<div class="col-sm-2">
								...
								<div class="row form-group">
									<label id="nova_atividade_mae" class="control-label col-sm-2">Atividade</label>
									<div class="col-md-2">
										<div id="id">
											<label class="control-label">ID atividade</label>
											<input class="form-control" type="text" value="" readonly>
										</div>
										<div id="id_atividade_mae">
											<label class="control-label">ID atividade mãe</label>
											<input class="form-control" type="text" value="" readonly>
										</div>
									</div>
									<div class="col-md-9">
										<label id="atividade_mae_atual" class="control-label">Atividade</label>
										<textarea name="atividade" class="form-control empresa" rows="5"></textarea>
										<span class="help-block"></span>
									</div>
								</div>
					...
					<div class="modal-footer">
						<button type="button" id="btnSave1" onclick="save_mae()" class="btn btn-success">Salvar</button>
						<button type="button" class="btn btn-default" data-dismiss="modal">Cancelar</button>
					</div>
				</div><!-- /.modal-content -->
			</div><!-- /.modal-dialog -->
		...
		<div class="modal fade" id="modal_form" role="dialog">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					<div class="modal-header">
						<div style="float: right;">
							<button type="button" id="btnSaveFilha" onclick="save()" class="btn btn-success">
								Atribuir
							</button>
							<button type="button" class="btn btn-default" data-dismiss="modal">Cancelar</button>
						</div>
						...
						<form action="#" id="form_filha" class="form-horizontal">
							<div class="form-body">
								...
								<div class="row form-group">
									...
									<div class="col-md-9">
										<textarea name="atividade_mae" class="form-control" rows="5"
												  readonly></textarea>
										<span class="help-block"></span>
									</div>
								</div>
								<div class="row form-group">
									<label class="control-label col-md-2">Atividade filha (a ser atribuída)</label>
									<div class="col-md-9">
										<textarea name="atividade" class="form-control" rows="5"></textarea>
										<span class="help-block"></span>
									</div>
								</div>
								<div class="row form-group">
					...
					<div class="modal-footer">
						<button type="button" id="btnSaveFilha1" onclick="save()" class="btn btn-success">
							Salvar
						</button>
						<button type="button" class="btn btn-default" data-dismiss="modal">Cancelar</button>
					</div>
...
<script>
	...
	function add_atividade() {
		$('#form_mae')[0].reset();
		$('#form_mae input[type="hidden"]').val('');
		$('#form_mae .form-group').removeClass('has-error');
		$('#form_mae .help-block').empty();
		$('#colaborador, #id, #id_atividade_mae, #atividade_mae_atual').hide();
		$('#nova_atividade_mae').show();
		$('input.empresa, textarea.empresa').prop('readonly', false);
		$('select.empresa').prop('disabled', false);
		$('#modal_form_mae .modal-title').text('Cadastrar atividade mãe');
		$('#modal_form_mae').modal('show');
		save_method = 'add';
	}
	...
	function edit_atividade_mae(id) {
		$.ajax({
			...
			'success': function (json) {
				...
				$('#modal_form_mae [name="id"]').val(json.id);
				$('#modal_form_mae [name="id_usuario"]').val(json.id_usuario);
				$('#modal_form_mae [name="id_mae"]').val(json.id_mae);
				$('#modal_form_mae [name="id_usuario"]').val(json.id_usuario);
				$('#modal_form_mae [name="atividade"]').val(json.atividade);
				$('#modal_form_mae [name="prioridade"]').val(json.prioridade);
				$('#modal_form_mae [name="tipo"]').val(json.tipo);
				$('#modal_form_mae [name="data_limite"]').val(json.data_limite);
				$('#modal_form_mae [name="data_lembrete"]').val(json.data_lembrete);
				$('#modal_form_mae [name="observacoes"]').val(json.observacoes);
				$('#id').show().find('input').val(json.id);
				$('#id_atividade_mae').hide().find('input').val(json.id_mae);
				$('#nova_atividade_mae').hide();
				$('#atividade_mae_atual').show();

				$('#modal_form_mae .modal-title').text('Editar atividade mãe');
				$('#modal_form_mae').modal('show');
				$('#modal_form_mae .modal-title');
				save_method = 'update';
			},
	...
	function add_atividade_filha(id) {
		...
		$.ajax({
			...
			'success': function (json) {
				...
				$('#modal_form [name="id_mae"]').val(json.id);
				$('#modal_form [name="atividade_mae"]').val(json.atividade);
				$('#modal_form [name="atividade"]').val('');
	...
	function edit_atividade_filha(id) {
		$.ajax({
			...
			'success': function (json) {
				...
				$('#modal_form_mae .modal-title').text('Editar atividade filha');
				$('#modal_form_mae').modal('show');
				$('#modal_form_mae .modal-title');
				save_method = 'update';
			},
	...
	function save() {
		$.ajax({
			...
			'dataType': 'json',
			'beforeSend': function () {
				$('#btnSaveFilha, #btnSaveFilha1').text('Atribuindo...').attr('disabled', true);
			},
			'success': function (json) {
				if (json.erro) {
					alert(json.erro);
			...
			'complete': function () {
				$('#btnSaveFilha, #btnSaveFilha1').text('Atribuir').attr('disabled', false);
			}
		});
	}
