							...
							<div class="row">
								<div class="col-md-4">
									...
									<select id="status" class="form-control filtro input-sm"
										...
										<option value="A,G,P,R" selected>Aguardando aprovação + Ativas + Fechadas
											parcialmente + Pendências na RP
										</option>
						...
						<form action="#" id="form" class="form-horizontal">
							...
							<div class="form-body">
								...
								<div class="row form-group">
									...
									<div class="col-md-2">
										...
										<input name="data_processo_seletivo" placeholder="dd/mm/aaaa"
											   class="form-control text-center data gestao_pessoas" type="text">
										<br>
										<button id="informar_pendencia" type="button" class="btn btn-warning"
												onclick="informar_pendencia_requisicao();">Informar requisitante sobre
											pendência
										</button>
									</div>
								</div>
								...
								<div id="dados_cuidadores" style="display: none;">
									<div class="well well-sm indicado possui_indicacao">
										<input name="id_indicado[]" type="hidden">
										<div class="row form-group">
											<label class="control-label col-md-2 nome_indicado">Indicado 1<span
													class="text-danger label_indicacao"> *</span></label>
											<div class="col-md-9">
												<input name="nome_indicado[]" type="text" class="form-control"
										...
										<div class="row form-group">
											<label class="control-label col-md-2">Nome da mãe<span
													class="text-danger label_indicacao"> *</span></label>
											<div class="col-md-9">
												<input name="nome_mae_indicado[]" type="text" class="form-control"
										...
										<div class="row form-group">
											<label class="control-label col-md-2">Nome do pai<span
													class="text-danger label_indicacao"> *</span></label>
											<div class="col-md-9">
												<input name="nome_pai_indicado[]" type="text" class="form-control"
										...
										<div class="row form-group">
											<label class="control-label col-md-2">Data nascimento<span
													class="text-danger label_indicacao"> *</span></label>
											<div class="col-md-2">
												<input name="data_nascimento_indicado[]" type="text"
											...
											<label class="control-label col-md-1">RG<span
													class="text-danger label_indicacao"> *</span></label>
											<div class="col-md-2">
												<input name="rg_indicado[]" type="text"
													   class="form-control rg">
											</div>
											<label class="control-label col-md-2">Data emissão RG<span
													class="text-danger label_indicacao"> *</span></label>
											<div class="col-md-2">
												<input name="rg_data_emissao_indicado[]" type="text"
													   class="form-control text-center data"
													   placeholder="dd/mm/aaaa">
											</div>
										</div>
										<div class="row form-group">
											<label class="control-label col-md-2 text-nowrap">Órgão emissor RG<span
													class="text-danger label_indicacao"> *</span></label>
											<div class="col-md-2">
												<input name="rg_orgao_emissor_indicado[]" type="text"
													   class="form-control">
											</div>
											<label class="control-label col-md-1">CPF<span
													class="text-danger label_indicacao"> *</span></label>
											<div class="col-md-2">
												<input name="cpf_indicado[]" type="text"
													   class="form-control cpf">
											</div>
											<label class="control-label col-md-1">PIS<span
													class="text-danger label_indicacao"> *</span></label>
											<div class="col-md-2">
												<input name="pis_indicado[]" type="text"
													   class="form-control text-center pis">
											</div>
											<div class="col-sm-1 remover_indicado" style="display: none;">
												<button type="button" style="border-radius: 14px;"
														class="btn btn-danger btn-sm"
														onclick="remover_indicado(this);">Remover
												</button>
											</div>
										</div>
									</div>
									<div class="row" id="adicionar_indicado">
										<div class="col-sm-10 col-sm-offset-2">
											<button type="button" style="border-radius: 14px;"
													class="btn btn-info btn-sm"
													onclick="add_indicado();">
												<i class="glyphicon glyphicon-plus"></i> Adicionar indicado
											</button>
										</div>
										<hr>
									</div>
								...
								<div id="dados_cuidadores_old" style="display: none;">
									<hr>
								...
								<div class="row form-group">
									...
									<div class="col-md-9">
										...
										<span
											class="text-primary">Ex.: DE-Suzano / 009/2014 ou EMTU - 015/2011</span>
									</div>
								</div>
								<div class="row form-group">
								...
								<div class="row form-group">
									<label class="control-label col-md-2">Previsão início <span
											class="text-danger">*</span></label>
									<div class="col-md-2">
...
<script>
    ...
    $(document).ready(function () {
        ...
        table = $('#table').DataTable({
                    ...
                    if (json.draw === 1) {
                        if (tipo_empresa) {
                            $("#dt_filtro").html('<br>Mostrar:&ensp;<label class="checkbox-inline">' +
                                ...
                                '</label>' +
                                '<label class="checkbox-inline">' +
                                '<input type="checkbox" onchange="toggle_vis(4)"> Selecionador(a)' +
                                '</label>' +
                                '<label class="checkbox-inline">' +
                                '<input type="checkbox" onchange="toggle_vis(11)"> Cargo/função' +
                                '</label>' +
                                '<label class="checkbox-inline">' +
                                '<input type="checkbox" onchange="toggle_vis(10)"> Previsão início' +
                                '</label>');
                        } else {
                            $("#dt_filtro").html('<br>Mostrar:&ensp;<label class="checkbox-inline">' +
                                '<input type="checkbox" onchange="toggle_vis(3)"> Estágio' +
            ...
            'columnDefs': [
                {
                    'visible': false,
                    'targets': tipo_empresa ? [3, 4, 7, 9, 10, 11, 12, 13] : [2, 3, 4, 9, 10, 11]
                },
                {
        ...
        if (<?= ($id_depto === '7' or $depto === 'Gestão de Pessoas') ? 'false' : 'true'; ?>) {
            ...
            $('#informar_pendencia').prop('disabled', true).removeAttr('onclick');
        }
    ...
    $('[name="status"]').on('change', function () {
        ...
        if (this.value === 'F') {
            ...
            if (<?= ($id_depto === '7' or $depto === 'Gestão de Pessoas') ? 'true' : 'false'; ?>) {
                $('#form [name="data_processo_seletivo"]').prop('readonly', true);
            }
            ...
            if (<?= ($id_depto === '7' or $depto === 'Gestão de Pessoas') ? 'true' : 'false'; ?>) {
                $('#informar_pendencia').prop('disabled', false);
            }
        } else {
            if (<?= ($id_depto === '7' or $depto === 'Gestão de Pessoas') ? 'true' : 'false'; ?>) {
                $('#form [name="data_processo_seletivo"]').prop('readonly', false);
                $('#informar_pendencia').prop('disabled', true);
            }
    ...
    $('.estrutura').on('change', function () {
        ...
        if (this.id === 'depto') {
            if (this.value === '5' && $('#possui_indicacao').is(':checked')) { // Cuidadores e Possui indicação
                $('#dados_cuidadores, #dados_cuidadores_old').show();
                ...
                //     $('.gestao_pessoas').hide();
                // }
                $('#dados_cuidadores, #dados_cuidadores_old').hide();
                $('#dados_cuidadores input, #dados_cuidadores_old input').val('');
            }
        }
    });
    ...
    function edit_requisicao(id) {
        ...
        $.ajax({
            ...
            'success': function (json) {
                ...
                $.each(indicados, function (id, indicado) {
                    add_indicado();
                    $.each(indicado, function (key, value) {
                        $('#form .indicado:last [name="' + key + '"][]').val(value);
                    });
                });
    ...
    function informar_pendencia_requisicao() {
        $.ajax({
            'url': '<?php echo site_url('requisicaoPessoal/informarPendencia') ?>',
            'type': 'POST',
            'dataType': 'json',
            'data': {
                'id': $('#form [name="id"]').val(),
                'requisitante_interno': $('#form [name="requisitante_interno"]').val(),
                'descricao_pendencias': $('#form [name="descricao_pendencias"]').val()
            },
            'beforeSend': function () {
                $('#informar_pendencia').text('Informando pendência ao requisitante...').prop('disabled', true);
            },
            'success': function (json) {
                if (json.erro) {
                    alert(json.erro);
            ...
            'complete': function () {
                $('#informar_pendencia').text('Informar requisitante sobre pendência').prop('disabled', false);
            }
        });
    }
    ...
    function add_indicado() {
        $('<div class="well well-sm indicado">' + $('.indicado:last').html() + '</div>').insertAfter('.indicado:last');
        $('.indicado:last span').removeClass('text-danger').addClass('text-info');
        $('.nome_indicado:last').text('Indicado ' + $('.indicado').length);
        $('.remover_indicado:last').show();
    }


    function remover_indicado(event) {
        $(event).parents('.well.indicado').remove();
        $('.indicado').each(function (i) {
            $('.nome_indicado:eq(' + i + ')').text('Indicado ' + (i + 1));
        });
    }

    function reset_indicados() {
        $('.well.indicado:gt(0)').remove();
        $('.well.indicado:first input').val('');
    }
