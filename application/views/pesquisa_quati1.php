            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <ol class="breadcrumb" style="margin-bottom: 5px; background-color: #eee;">
                        ...
                        <?php elseif ($tipo == 'quati'): ?>
                            <li class="active">Testes de Personalidade - Tipologia Junguiana</li>
                        <?php else: ?>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            table = $('#table').DataTable({
                ...
                "ajax": {
                    "url": "<?php echo site_url('pesquisa/ajaxQuati/' . $empresa) ?>",
                    "type": "POST"
                },
        ...
        function edit_pesquisa(id) {
            ...
            $.ajax({
                url: "<?php echo site_url('pesquisa/ajax_editQuati/') ?>/" + id,
                type: "GET",
                dataType: "JSON",
        ...
        function save() {
            var url;
            if (save_method === 'add') {
                url = "<?php echo site_url('pesquisa/ajax_addQuati') ?>";
            } else if (save_method === 'update') {
                url = "<?php echo site_url('pesquisa/ajax_updateQuati') ?>";
        ...
        function delete_avaliador(id) {
            if (confirm('Deseja remover o(a) colaborador(a) da avaliação?')) {
                // ajax delete data to database
                $.ajax({
                    url: "<?php echo site_url('pesquisa/ajax_deleteQuati') ?>/" + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function (data) {
                        //if success reload ajax table
