    ...
    <section class="wrapper">
        ...
        <table class="table table-condensed table-condensed avaliacao">
            ...
            <tr style='border-top: 5px solid #ddd;'>
                <th colspan="3">
                    <h3 class="text-center">RELATÓRIO DE PROPOSTAS</h3>
                </th>
            </tr>
            </thead>
            <tbody>
            <tr style='border-top: 5px solid #ddd; border-bottom: 1px solid #ddd;'>
                ...
                <td class="text-right">
                    <a id="pdf" class="btn btn-sm btn-info"
                       href="<?= site_url('icom/propostas/pdf/q?' . http_build_query($this->input->get())); ?>"
            ...
            <?php if ($rows): ?>
                <?php foreach ($rows as $row): ?>
                    <tr>
                        ...
                        <td width="20%"><?= $row->nome_cliente; ?></td>
                        <td width="20%"><?= $row->descricao; ?></td>
                        <td width="20%"><?= $row->contato_principal; ?></td>
                        <td width="20%"><?= $row->telefone_contato_principal; ?></td>
                        <td width="20%"><?= $row->email_contato_principal; ?></td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
                <tr>
                    <td class="text-center text-muted" colspan="7">Nenhum registro encontrado</td>
                </tr>
            <?php endif; ?>
            </tbody>
        </table>
...
<script>
    $(document).ready(function () {
        document.title = 'CORPORATE RH - LMS - Relatório de Propostas';
    });
</script>
