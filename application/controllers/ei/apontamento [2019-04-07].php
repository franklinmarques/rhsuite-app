...
//        $this->db->where('a.id_usuario', $where['supervisor']);
    ...
    public function iniciarSemestre()
    {
        ...
        $this->db->select('b.nome AS escola, b.municipio, c.nome AS ordem_servico, d.contrato', false);
        ...
        $this->db->select('b.status, b.hipotese_diagnostica, a.data_inicio, a.data_termino', false);
        ...
        $this->db->select(['a.data_inicio_contrato, a.data_termino_contrato, a.valor_hora_operacional, a.horas_mensais_custo'], false);
        $this->db->select(['l.valor AS valor_hora_funcao, l.valor_pagamento AS valor_hora_pagamento'], false);
        ...
        $this->db->join('ei_valores_faturamento l', 'l.id_contrato = k.id AND l.id_cargo = e.id AND l.id_funcao = f.id', 'left');
        $this->db->where('d.id_alocacao', $idAlocacao);
    ...
    public function adicionarOSIndividual()
    {
        ...
        $sqlProfissionais = "SELECT '{$alocacao->id}' AS id_alocacao,
                                    ...
                                    e.contrato,
                                    a.valor_hora,
                                    IFNULL(a.valor_hora_operacional, l.valor) AS valor_hora_operacional,
                                    l.valor_pagamento AS valor_hora_pagamento,
                                    a.horas_diarias,
                                    a.horas_semanais,
                                    a.qtde_dias,
                                    a.horas_semestre
...
//                <strong>Qtde. de dias letivos no semestre</strong> {$alocado->total_dias_letivos}
    ...
    public function ajaxListControleMateriais()
    {
        ...
        $this->db->select('b.municipio, a.aluno, b.escola, b.ordem_servico, a.id, d.cuidador');
        $this->db->join('ei_alocacao_escolas b', 'b.id = a.id_alocacao_escola');
        $this->db->join('ei_alocacao c', 'c.id = b.id_alocacao');
        $this->db->join('ei_alocados d', 'd.id_alocacao_escola = b.id');
        $this->db->where('c.id_empresa', $this->session->userdata('empresa'));
        $this->db->where('c.depto', $busca['depto']);
        $this->db->where('c.id_diretoria', $busca['diretoria']);
        $this->db->where('c.id_supervisor', $busca['supervisor']);
...
//        }
    ...
    public function ajaxSaveFaturamentoConsolidado()
    {
        ...
        $this->db->select('id');
        $this->db->where('depto', $this->input->post('depto'));
        $this->db->where('id_diretoria', $this->input->post('diretoria'));
        $this->db->where('id_supervisor', $this->input->post('supervisor'));
        $this->db->where('ano', $this->input->post('ano'));
        $this->db->where('semestre', $this->input->post('semestre'));
        $alocacao = $this->db->get('ei_alocacao')->row();
...
//        $this->db->join('ei_apontamento c', "c.id_alocado  = b.id AND c.periodo = a.periodo", 'left');
    ...
    public function planilhaFaturamento($idAlocado, $idMes, $periodo, $is_pdf = false, $recuperar = false)
    {
        ...
        $this->db->select('a.cuidador, b.contrato, b.id_escola, b.escola, b.ordem_servico, c.funcao, b2.semestre, b2.ano');
        ...
        $this->db->group_by(['a.id', 'b.id_escola', 'd.cargo', 'd.funcao']);
        $rows = $this->db->get('ei_alocados a')->result();


        /*$this->db->start_cache();
        $this->db->select('d.id_alocado, b.contrato, b.escola, b.ordem_servico');
        $this->db->select("(CASE d.periodo WHEN 0 THEN 'Madrugada' WHEN 1 THEN 'Manhã' WHEN 2 THEN 'Tarde' WHEN 3 THEN 'Noite' END) AS nome_periodo", false);
        $this->db->select('a.ano, a.semestre, d.cargo, d.funcao, i1.id AS id_totalizacao, c.cuidador');
        if ($substituto === '2') {
            if ($recuperar) {
                $this->db->select("(SELECT COUNT(IF(x.status = 'EU', 1, 0)) FROM ei_apontamento x WHERE x.id_alocado = b.id AND MONTH(x.data) = {$idMes}) + IFNULL(SUM(IF(MONTH(a.data_substituicao2) = ({$idMes} + IF(c.semestre > 1, 7, 0)), a.total_semanas_sub2 - a.desconto_sub2, null)), IF(a.periodo = '{$periodo}', a.total_semanas_mes{$idMes} - a.desconto_mes{$idMes}, NULL)) AS total_dias", false);
                $this->db->select("TIME_FORMAT(ADDTIME(b.total_horas_sub2_mes{$idMes}, IFNULL(b.horas_descontadas_sub2_mes{$idMes}, 0)), '%H:%i:%s') AS total_horas", false);
            } else {
            ...
            if ($recuperar) {
                $this->db->select("(SELECT x.total_dias_mes{$idMes} FROM ei_alocados_totalizacao x WHERE x.id_alocado = c.id AND x.periodo = d.periodo) AS total_dias", false);
                $this->db->select("(SELECT TIME_FORMAT(SEC_TO_TIME(TIME_TO_SEC(x.total_horas_mes{$idMes}) + TIME_TO_SEC(d.desconto_mes{$idMes})), '%H:%i') FROM ei_alocados_totalizacao x WHERE x.id_alocado = c.id AND x.periodo = d.periodo) AS total_horas", false);
            } else {
                $this->db->select("IFNULL(i1.total_dias_mes{$idMes}, (SELECT x.total_dias_mes{$idMes} FROM ei_alocados_totalizacao x WHERE x.id_alocado = b.id AND x.periodo = d.periodo)) AS total_dias", false);
                $this->db->select("TIME_FORMAT(IFNULL(i1.total_horas_mes{$idMes}, (SELECT SEC_TO_TIME(TIME_TO_SEC(x.total_horas_mes{$idMes}) + TIME_TO_SEC(d.desconto_mes{$idMes})) FROM ei_alocados_totalizacao x WHERE x.id_alocado = b.id AND x.periodo = d.periodo)), '%H:%i') AS total_horas", false);
            }
        }
        $this->db->select(["DATE_FORMAT(IF(MONTH(MIN(h.data_inicio)) = ({$idMes} + IF(a.semestre = 2, 5, 0)), MIN(h.data_inicio), CONCAT(a.ano, '-', {$idMes} + IF(a.semestre = 2, 7, 0), '-1')), '%d/%m/%Y') AS periodo_inicial"], false);
        if ($idMes === '1' or $idMes === '7') {
            $this->db->select(["DATE_FORMAT(IFNULL(MAX(h.data_recesso),IFNULL(MAX(h.data_termino), IFNULL(DATE_SUB(MAX(d.data_substituicao1), INTERVAL 1 DAY), LAST_DAY(CONCAT(a.ano, '-', {$idMes} + IF(a.semestre = 2, 7, 0), '-1'))))), '%d/%m/%Y') AS periodo_final"], false);
        } else {
            $this->db->select(["DATE_FORMAT(IF(MONTH(MAX(h.data_termino)) = ({$idMes} + IF(a.semestre = 2, 5, 0)), MAX(h.data_termino), LAST_DAY(CONCAT(a.ano, '-', {$idMes} + IF(a.semestre = 2, 7, 0), '-1'))), '%d/%m/%Y') AS periodo_final"], false);
        }
        $this->db->select("GROUP_CONCAT(DISTINCT h.aluno ORDER BY h.aluno SEPARATOR ', ') AS alunos", false);
        $this->db->select("MIN(d.dia_semana) AS dia_semana_inicial", false);
        $this->db->select("IF(COUNT(d.dia_semana) > 1, MAX(d.dia_semana), NULL) AS dia_semana_final", false);
        $this->db->select("TIME_FORMAT(d.horario_inicio, '%H:%ih') AS horario_inicio", false);
        $this->db->select("TIME_FORMAT(d.horario_termino, '%H:%ih') AS horario_termino", false);
        $this->db->join('ei_alocacao_escolas b', 'b.id_alocacao = a.id');
        $this->db->join('ei_alocados c', 'c.id_alocacao_escola = b.id');
        $this->db->join('ei_alocados_horarios d', 'd.id_alocado = c.id', 'left');
        $this->db->join('usuarios e', "e.id = d.id_cuidador_sub1 AND IF(MONTH(d.data_substituicao1) > 7, MONTH(d.data_substituicao1) - 7, MONTH(d.data_substituicao1)) = '{$idMes}'", 'left');
        $this->db->join('usuarios f', "f.id = d.id_cuidador_sub2 AND IF(MONTH(d.data_substituicao2) > 7, MONTH(d.data_substituicao2) - 7, MONTH(d.data_substituicao2)) = '{$idMes}'", 'left');
        $this->db->join('ei_alocados_totalizacao i1', 'i1.id_alocado = c.id AND i1.periodo = d.periodo', 'left');
        $this->db->join('ei_matriculados_turmas g', 'g.id_alocado_horario = d.id', 'left');
        $this->db->join('ei_matriculados h', 'h.id = g.id_matriculado AND h.id_alocacao_escola = b.id', 'left');
        $this->db->join('ei_faturamento i', 'i.id_alocacao = a.id and i.funcao = d.funcao', 'left');
        $this->db->where('a.id', $alocados->id_alocacao);
        $this->db->where('b.id_escola', $alocados->id_escola);
        $this->db->where('d.funcao', $alocados->funcao);
        $this->db->stop_cache();
        $this->db->group_by(['b.id_escola', 'd.cargo', 'd.funcao']);
        $data = $this->db->get('ei_alocacao a')->row();


        $this->db->group_by(['b.id_escola', 'd.cargo', 'd.funcao', 'd.periodo', 'c.id']);
        $rows = $this->db->get('ei_alocacao a')->result();
        $this->db->flush_cache();*/
//----------------------------------

        /*$this->db->start_cache();
        $this->db->select('a.id_alocado, b1.contrato, b1.escola, b1.ordem_servico');
        $this->db->select("(CASE a.periodo WHEN 0 THEN 'Madrugada' WHEN 1 THEN 'Manhã' WHEN 2 THEN 'Tarde' WHEN 3 THEN 'Noite' END) AS nome_periodo", false);
        if ($substituto === '2') {
            $this->db->select("c.ano, c.semestre, GROUP_CONCAT(DISTINCT a.funcao_sub2 ORDER BY a.funcao_sub2 SEPARATOR ', ') AS funcao", false);
            $this->db->select("GROUP_CONCAT(DISTINCT a.cargo_sub2 ORDER BY a.cargo_sub2 SEPARATOR ', ') AS cargo, k.id AS id_totalizacao", false);
            if ($recuperar) {
                $this->db->select("(SELECT COUNT(IF(x.status = 'EU', 1, 0)) FROM ei_apontamento x WHERE x.id_alocado = b.id AND MONTH(x.data) = {$idMes}) + IFNULL(SUM(IF(MONTH(a.data_substituicao2) = ({$idMes} + IF(c.semestre > 1, 7, 0)), a.total_semanas_sub2 - a.desconto_sub2, null)), IF(a.periodo = '{$periodo}', a.total_semanas_mes{$idMes} - a.desconto_mes{$idMes}, NULL)) AS total_dias", false);
                $this->db->select("TIME_FORMAT(ADDTIME(b.total_horas_sub2_mes{$idMes}, IFNULL(b.horas_descontadas_sub2_mes{$idMes}, 0)), '%H:%i:%s') AS total_horas", false);
//                $this->db->select("TIME_FORMAT(b.total_horas_sub2_mes{$idMes}, '%H:%i:%s') AS total_horas", false);
            } else {
                $this->db->select("(SELECT COUNT(IF(x.status = 'EU', 1, 0)) FROM ei_apontamento x WHERE x.id_alocado = b.id AND MONTH(x.data) = {$idMes}) + IFNULL(k.total_dias_sub2, IFNULL(SUM(IF(MONTH(a.data_substituicao2) = ({$idMes} + IF(c.semestre > 1, 7, 0)), a.total_semanas_sub2 - a.desconto_sub2, null)), IF(a.periodo = '{$periodo}', a.total_semanas_mes{$idMes} - a.desconto_mes{$idMes}, NULL))) AS total_dias", false);
                $this->db->select("IFNULL(k.total_horas_sub2, TIME_FORMAT(ADDTIME(b.total_horas_sub2_mes{$idMes}, IFNULL(b.horas_descontadas_sub2_mes{$idMes}, 0)), '%H:%i:%s')) AS total_horas", false);
            }
            $this->db->select("DATE_FORMAT(IFNULL(MAX(a.data_substituicao2), CONCAT(c.ano, '-', {$idMes} + IF(c.semestre = 2, 7, 0), '-1')), '%d/%m/%Y') AS periodo_inicial", false);
            $this->db->select("GROUP_CONCAT(DISTINCT a3.nome ORDER BY a3.nome SEPARATOR ', ') AS cuidador", false);
            if ($idMes === '1' or $idMes === '6') {
                $this->db->select("DATE_FORMAT(IFNULL(MAX(l.data_recesso), LAST_DAY(CONCAT(c.ano, '-', {$idMes} + IF(c.semestre = 2, 7, 0), '-1'))), '%d/%m/%Y') AS periodo_final", false);
            } else {
                $this->db->select("DATE_FORMAT(LAST_DAY(CONCAT(c.ano, '-', {$idMes} + IF(c.semestre = 2, 7, 0), '-1')), '%d/%m/%Y') AS periodo_final", false);
            }
        } elseif ($substituto === '1') {
            $this->db->select("c.ano, c.semestre, a.cargo_sub1 AS cargo, a.funcao_sub1 AS funcao, k.id AS id_totalizacao", false);
            if ($recuperar) {
                $this->db->select("(SELECT COUNT(IF(x.status = 'EU', 1, 0)) FROM ei_apontamento x WHERE x.id_alocado = b.id AND MONTH(x.data) = {$idMes}) + IFNULL(SUM(IF(MONTH(a.data_substituicao1) = ({$idMes} + IF(c.semestre > 1, 7, 0)), a.total_semanas_sub1 - a.desconto_sub1, null)), IF(a.periodo = '{$periodo}', a.total_semanas_mes{$idMes} - a.desconto_mes{$idMes}, NULL)) AS total_dias", false);
                $this->db->select("TIME_FORMAT(ADDTIME(b.total_horas_sub1_mes{$idMes}, IFNULL(b.horas_descontadas_sub1_mes{$idMes}, 0)), '%H:%i:%s') AS total_horas", false);
//                $this->db->select("TIME_FORMAT(b.total_horas_sub1_mes{$idMes}, '%H:%i:%s') AS total_horas", false);
            } else {
                $this->db->select("(SELECT COUNT(IF(x.status = 'EU', 1, 0)) FROM ei_apontamento x WHERE x.id_alocado = b.id AND MONTH(x.data) = {$idMes}) + IFNULL(k.total_dias_sub1, IFNULL(SUM(IF(MONTH(a.data_substituicao1) = ({$idMes} + IF(c.semestre > 1, 7, 0)), a.total_semanas_sub1 - a.desconto_sub1, null)), IF(a.periodo = '{$periodo}', a.total_semanas_mes{$idMes} - a.desconto_mes{$idMes}, NULL))) AS total_dias", false);
                $this->db->select("IFNULL(k.total_horas_sub1, TIME_FORMAT(ADDTIME(b.total_horas_sub1_mes{$idMes}, IFNULL(b.horas_descontadas_sub1_mes{$idMes}, 0)), '%H:%i:%s')) AS total_horas", false);
            }
            $this->db->select("DATE_FORMAT(IFNULL(MAX(a.data_substituicao1), CONCAT(c.ano, '-', {$idMes} + IF(c.semestre = 2, 7, 0), '-1')), '%d/%m/%Y') AS periodo_inicial", false);
            $this->db->select("GROUP_CONCAT(DISTINCT a2.nome ORDER BY a2.nome SEPARATOR ', ') AS cuidador", false);
            if ($idMes === '1' or $idMes === '6') {
                $this->db->select("DATE_FORMAT(IFNULL(MAX(l.data_recesso), IFNULL(DATE_SUB(MAX(a.data_substituicao2), INTERVAL 1 DAY), LAST_DAY(CONCAT(c.ano, '-', {$idMes} + IF(c.semestre = 2, 7, 0), '-1')))), '%d/%m/%Y') AS periodo_final", false);
            } else {
                $this->db->select("DATE_FORMAT(IFNULL(DATE_SUB(MAX(a.data_substituicao2), INTERVAL 1 DAY), LAST_DAY(CONCAT(c.ano, '-', {$idMes} + IF(c.semestre = 2, 7, 0), '-1'))), '%d/%m/%Y') AS periodo_final", false);
            }
        } else {
            $this->db->select('c.ano, c.semestre, a.cargo, a.funcao, k.id AS id_totalizacao, b.cuidador');
//            $this->db->select(["GROUP_CONCAT(DISTINCT b.cuidador ORDER BY b.cuidador SEPARATOR ', ') AS cuidador"], false);
            if ($recuperar) {
//                $this->db->select("(SELECT COUNT(IF(x.status = 'EU', 1, 0)) FROM ei_apontamento x WHERE x.id_alocado = b.id AND MONTH(x.data) = {$idMes}) + SUM(IF(a.periodo = '{$periodo}', a.total_semanas_mes{$idMes} - a.desconto_mes{$idMes}, NULL)) AS total_dias", false);
//                $this->db->select("TIME_FORMAT(ADDTIME(b.total_horas_mes{$idMes}, IFNULL(b.horas_descontadas_mes{$idMes}, 0)), '%H:%i:%s') AS total_horas", false);
//                $this->db->select("TIME_FORMAT(b.total_horas_mes{$idMes}, '%H:%i:%s') AS total_horas", false);
                $this->db->select("(SELECT x.total_dias_mes{$idMes} FROM ei_alocados_totalizacao x WHERE x.id_alocado = b.id AND x.periodo = a.periodo) AS total_dias", false);
                $this->db->select("(SELECT TIME_FORMAT(x.total_horas_mes{$idMes}, '%H:%i') FROM ei_alocados_totalizacao x WHERE x.id_alocado = b.id AND x.periodo = a.periodo) AS total_horas", false);
            } else {
//                $this->db->select("(SELECT COUNT(IF(x.status = 'EU', 1, 0)) FROM ei_apontamento x WHERE x.id_alocado = b.id AND MONTH(x.data) = {$idMes}) + IFNULL(k.total_dias_mes{$idMes}, SUM(IF(a.periodo = '{$periodo}', a.total_semanas_mes{$idMes} - a.desconto_mes{$idMes}, NULL))) AS total_dias", false);
                $this->db->select("IFNULL(k.total_dias_mes{$idMes}, (SELECT x.total_dias_mes{$idMes} FROM ei_alocados_totalizacao x WHERE x.id_alocado = b.id AND x.periodo = a.periodo)) AS total_dias", false);
                $this->db->select("TIME_FORMAT(IFNULL(k.total_horas_mes{$idMes}, (SELECT x.total_horas_mes{$idMes} FROM ei_alocados_totalizacao x WHERE x.id_alocado = b.id AND x.periodo = a.periodo)), '%H:%i') AS total_horas", false);
//                $this->db->select("IFNULL(k.total_horas_mes{$idMes}, TIME_FORMAT(ADDTIME(b.total_horas_mes{$idMes}, IFNULL(b.horas_descontadas_mes{$idMes}, 0)), '%H:%i:%s')) AS total_horas", false);
            }

//            $this->db->select("DATE_FORMAT(MIN(l.data_inicio), '%d/%m/%Y') AS periodo_inicial", false);
//            $this->db->select("DATE_FORMAT(MAX(l.data_termino), '%d/%m/%Y') AS data_termino_aluno", false);
//            $this->db->select("IFNULL(DATE_FORMAT(MAX(l.data_recesso), '%d/%m/%Y'), '00/00/0000') AS data_recesso_aluno", false);
//            $this->db->select("DATE_FORMAT(IFNULL(MIN(l.data_inicio), CONCAT(c.ano, '-', {$idMes} + IF(c.semestre = 2, 7, 0), '-1')), '%d/%m/%Y') AS periodo_inicial", false);
            $this->db->select(["DATE_FORMAT(IF(MONTH(MIN(l.data_inicio)) = ({$idMes} + IF(c.semestre = 2, 5, 0)), MIN(l.data_inicio), CONCAT(c.ano, '-', {$idMes} + IF(c.semestre = 2, 7, 0), '-1')), '%d/%m/%Y') AS periodo_inicial"], false);
            if ($idMes === '1' or $idMes === '7') {
                $this->db->select(["DATE_FORMAT(IFNULL(MAX(l.data_recesso),IFNULL(MAX(l.data_termino), IFNULL(DATE_SUB(MAX(a.data_substituicao1), INTERVAL 1 DAY), LAST_DAY(CONCAT(c.ano, '-', {$idMes} + IF(c.semestre = 2, 7, 0), '-1'))))), '%d/%m/%Y') AS periodo_final"], false);
            } else {
                $this->db->select(["DATE_FORMAT(IF(MONTH(MAX(l.data_termino)) = ({$idMes} + IF(c.semestre = 2, 5, 0)), MAX(l.data_termino), LAST_DAY(CONCAT(c.ano, '-', {$idMes} + IF(c.semestre = 2, 7, 0), '-1'))), '%d/%m/%Y') AS periodo_final"], false);
//                $this->db->select("DATE_FORMAT(IFNULL(DATE_SUB(MAX(a.data_substituicao1), INTERVAL 1 DAY), LAST_DAY(CONCAT(c.ano, '-', {$idMes} + IF(c.semestre = 2, 6, 0), '-1'))), '%d/%m/%Y') AS periodo_final", false);
            }
        }
        $this->db->select("GROUP_CONCAT(DISTINCT l.aluno ORDER BY l.aluno SEPARATOR ', ') AS alunos", false);
//        $this->db->select(["(SELECT GROUP_CONCAT(DISTINCT jj.aluno ORDER BY jj.aluno SEPARATOR ', ') FROM ei_matriculados jj WHERE jj.id_alocacao = c.id AND jj.escola = b1.escola AND jj.municipio = b1.municipio AND jj.ordem_servico = b1.ordem_servico) AS alunos"], false);
        $this->db->select("MIN(a.dia_semana) AS dia_semana_inicial", false);
        $this->db->select("IF(COUNT(a.dia_semana) > 1, MAX(a.dia_semana), NULL) AS dia_semana_final", false);


        $this->db->select("TIME_FORMAT(a.horario_inicio, '%H:%ih') AS horario_inicio", false);
        $this->db->select("TIME_FORMAT(a.horario_termino, '%H:%ih') AS horario_termino", false);


        $this->db->join('ei_alocados b', 'b.id = a.id_alocado');
        $this->db->join('ei_alocacao_escolas b1', 'b1.id = b.id_alocacao_escola');
        $this->db->join('ei_alocacao c', 'c.id = b1.id_alocacao');
        $this->db->join('usuarios a2', "a2.id = a.id_cuidador_sub1 AND IF(MONTH(a.data_substituicao1) > 7, MONTH(a.data_substituicao1) - 7, MONTH(a.data_substituicao1)) = '{$idMes}'", 'left');
        $this->db->join('usuarios a3', "a3.id = a.id_cuidador_sub2 AND IF(MONTH(a.data_substituicao2) > 7, MONTH(a.data_substituicao2) - 7, MONTH(a.data_substituicao2)) = '{$idMes}'", 'left');
        $this->db->join('ei_ordem_servico_profissionais d', 'd.id = b.id_os_profissional', 'left');
        $this->db->join('ei_ordem_servico_escolas e', 'e.id = d.id_ordem_servico_escola', 'left');
        $this->db->join('ei_ordem_servico f', 'f.id = e.id_ordem_servico', 'left');
        $this->db->join('ei_contratos g', 'g.id = f.id_contrato', 'left');
        $this->db->join('ei_ordem_servico_horarios d2', 'd2.id_os_profissional = d.id AND d2.id = a.id_os_horario', 'left');
        $this->db->join('ei_ordem_servico_turmas h', 'h.id_os_horario = d2.id', 'left');
        $this->db->join('ei_ordem_servico_alunos i', 'i.id = h.id_os_aluno AND i.id_ordem_servico_escola = e.id', 'left');
        $this->db->join('ei_alunos j', 'j.id = i.id_aluno', 'left');
        $this->db->join('ei_faturamento k', 'k.id_alocado = a.id_alocado AND k.periodo = a.periodo', 'left');
        $this->db->join('ei_matriculados l', 'l.id_alocacao_escola = b1.id AND l.id_os_aluno = i.id', 'left');
        $this->db->join('ei_alocados_totalizacao m', 'm.id_alocado = a.id_alocado AND m.periodo = a.periodo', 'left');
        $this->db->where('m.id IS NOT NULL', null, false);
        $this->db->where('c.id', $alocados->id_alocacao);
        $this->db->where('b1.escola', $alocados->escola);
        ...
        $this->db->group_by(['b1.escola', 'a.cargo', 'a.funcao', 'a.periodo', 'b.id']);
        $rows = $this->db->get('ei_alocados_horarios a')->result();
        $this->db->flush_cache();*/
        ...
        foreach ($horarioTrabalho as $horario) {
            $strHorario = $horario->id . '-' . $horario->periodo;
...
//            'profissional' => implode(', ', array_column($rows, 'cuidador')),
...
//        $semestre = $mes > 6 ? 2 : 1;
        ...
        $idSupervisor = $this->input->get_post('supervisor');
        $semestre = $this->input->get_post('semestre');


        $this->db->select('a.id, a.diretoria, null AS valor_hora', false);
        ...
        $subquery = "SELECT c.id_alocacao, a.total_horas_mes{$idMes}, d.cargo, d.funcao, d.valor_hora_funcao
...
//        $this->db->select('e.id, d.cargo, d.funcao, NULL AS total_segundos, NULL AS valor_total_individual', false);
//        if ($recuperar) {
//            $this->db->select("FORMAT(d.valor_hora_funcao, 2, 'de_DE') AS valor_hora", false);
//            $this->db->select("TIME_FORMAT(SEC_TO_TIME(IFNULL(TIME_TO_SEC(SUM(e.total_horas_mes{$idMes})), TIME_TO_SEC(SUM(d.total_mes{$idMes})))), '%H:%i') AS total_horas", false);
//            $this->db->select("FORMAT(valor_faturado_mes{$idMes} * (IFNULL(TIME_TO_SEC(SUM(e.total_horas_mes{$idMes})), TIME_TO_SEC(SUM(d.total_mes{$idMes}))) / 3600), 2, 'de_DE') AS valor_faturado", false);
//        } else {
//            $this->db->select("FORMAT(IFNULL(f.valor_hora_mes{$idMes}, d.valor_hora_funcao), 2, 'de_DE') AS valor_hora", false);
//            $this->db->select("TIME_FORMAT(SEC_TO_TIME(IFNULL(f.total_horas_mes{$idMes}, IFNULL(TIME_TO_SEC(SUM(e.total_horas_mes{$idMes})), TIME_TO_SEC(SUM(d.total_mes{$idMes}))))), '%H:%i') AS total_horas", false);
//            $this->db->select("FORMAT(f.valor_faturado_mes{$idMes} * (IFNULL(f.total_horas_mes{$idMes}, IFNULL(TIME_TO_SEC(SUM(e.total_horas_mes{$idMes})), TIME_TO_SEC(SUM(d.total_mes{$idMes})))) / 3600), 2, 'de_DE') AS valor_faturado", false);
//        }
//        $this->db->join('ei_alocacao_escolas b', 'b.id_alocacao = a.id');
//        $this->db->join('ei_alocados c', 'c.id_alocacao_escola = b.id', 'left');
//        $this->db->join('ei_alocados_horarios d', 'd.id_alocado = c.id', 'left');
//        $this->db->join('ei_alocados_totalizacao e', 'e.id_alocado = c.id AND e.periodo = d.periodo', 'left');
//        $this->db->join('ei_faturamento_consolidado f', 'f.id_alocacao = a.id AND f.cargo = d.cargo AND f.funcao = d.funcao', 'left');
//        $this->db->where('a.id', $data->id);
//        $this->db->where('d.funcao IS NOT NULL', null, false);
//        $this->db->group_by(['a.id', 'd.funcao']);
//        $this->db->order_by('d.funcao', 'asc');
//        $alocados = $this->db->get('ei_alocacao a')->result();


        /*$this->db->select('c.id, d.nome AS diretoria');
        $this->db->select('COUNT(DISTINCT(b.escola)) AS total_escolas');
        $this->db->select('COUNT(DISTINCT(c.cuidador)) AS total_profissionais');
        $this->db->select('COUNT(DISTINCT(e.aluno)) AS total_alunos');
        $this->db->select(["GROUP_CONCAT(DISTINCT b.contrato ORDER BY b.contrato SEPARATOR ', ') AS contratos"], false);
        $this->db->select(["GROUP_CONCAT(DISTINCT b.ordem_servico ORDER BY b.ordem_servico SEPARATOR ', ') AS ordens_servico"], false);
        $this->db->join('ei_alocacao_escolas b', 'b.id_alocacao = a.id', 'left');
        $this->db->join('ei_alocados c', 'c.id_alocacao_escola = b.id', 'left');
        $this->db->join('ei_diretorias d', 'd.id = a.id_diretoria', 'left');
        $this->db->join('ei_matriculados e', 'e.id_alocacao_escola = b.id', 'left');
        $this->db->where('a.id_diretoria', $idDiretoria);
        $this->db->where('a.ano', $ano);
        $this->db->where('a.semestre', $semestre);
        $data = $this->db->get('ei_alocacao a')->row();
        ...
        $this->db->start_cache();
        if ($recuperar) {
            $this->db->select(["NULL AS id_totalizacao_consolidado, FORMAT(f.valor, 2, 'de_DE') AS valor_hora"], false);
            $this->db->select("SUM(TIME_TO_SEC(IFNULL(h.total_horas_mes{$idMes}, h.total_horas_mes{$idMes}))) AS total_segundos", false);
            $this->db->select(["FORMAT(f.valor * SUM(TIME_TO_SEC(IFNULL(h.total_horas_mes{$idMes}, h.total_horas_mes{$idMes})) / 3600), 2, 'de_DE') AS valor_faturado"], false);
        } else {
            $this->db->select(["g.id AS id_totalizacao_consolidado, FORMAT(IFNULL(g.valor_hora_mes{$idMes} , f.valor), 2, 'de_DE') AS valor_hora"], false);
            $this->db->select("IFNULL(g.total_horas_mes{$idMes}, SUM(TIME_TO_SEC(IFNULL(h.total_horas_mes{$idMes}, h.total_horas_mes{$idMes})))) AS total_segundos", false);
            $this->db->select(["FORMAT(IFNULL(g.valor_faturado_mes{$idMes}, f.valor * SUM(TIME_TO_SEC(IFNULL(h.total_horas_mes{$idMes}, h.total_horas_mes{$idMes})) / 3600)), 2, 'de_DE') AS valor_faturado"], false);
        }
        $this->db->select(["b.id AS id_alocacao, f.valor * SUM(TIME_TO_SEC(IFNULL(h.total_horas_mes{$idMes}, h.total_horas_mes{$idMes})) / 3600) AS valor_total_individual"], false);
        $this->db->join('ei_alocacao_escolas a1', 'a1.id = a.id_alocacao_escola');
        $this->db->join('ei_alocacao b', 'b.id = a1.id_alocacao');
        $this->db->join('ei_alocados_horarios a2', 'a2.id_alocado = a.id', 'left');
        $this->db->join('empresa_funcoes c', 'c.nome = a2.funcao', 'left');
        $this->db->join('empresa_cargos c2', 'c2.nome = a2.cargo', 'left');
        $this->db->join('ei_ordem_servico d', 'd.nome = a1.ordem_servico AND d.ano = b.ano AND d.semestre = b.semestre', 'left');
        $this->db->join('ei_contratos e', 'e.id = d.id_contrato', 'left');
        $this->db->join('ei_valores_faturamento f', 'f.id_contrato = e.id AND f.ano = d.ano AND f.semestre = d.semestre AND f.id_cargo = c.id_cargo AND f.id_funcao = c.id', 'left');
        $this->db->join('ei_faturamento_consolidado g', 'g.id_alocacao = b.id AND g.cargo = c2.nome AND g.funcao = c.nome', 'left');
        $this->db->join('ei_faturamento h', 'h.id_alocado = a.id AND h.cargo = a2.cargo AND h.funcao = a2.funcao', 'left');
        $this->db->where('b.id_diretoria', $idDiretoria);
        $this->db->where('b.ano', $ano);
        $this->db->where('b.semestre', $semestre);
        $this->db->stop_cache();
        $somaAlocados = $this->db->get('ei_alocados a')->row();


        $this->db->select('a2.cargo, a2.funcao');
        $this->db->group_by('a2.cargo, a2.funcao');
        $this->db->order_by('a2.funcao', 'asc');
        $alocados = $this->db->get('ei_alocados a')->result();
        $this->db->flush_cache();*/
        ...
        $planilha = array(
            ...
            'total_horas' => secToTime($totalHoras, true),
    ...
    public function planilhaPagamentoPrestador1($idHorario, $mes, $ano, $is_pdf = false, $recuperar = false)
    {
        $this->db->select('foto, foto_descricao');
        $this->db->where('id', $this->session->userdata('empresa'));
        $empresa = $this->db->get('usuarios')->row();
        ...
        $idMes = intval($mes) > 6 ? intval($mes) - 6 : intval($mes);
        $mes = str_pad($mes, 2, '0', STR_PAD_LEFT);


//        if ($substituto === '2') {
//            $this->db->select("GROUP_CONCAT(DISTINCT e.nome ORDER BY e.nome SEPARATOR ', ') AS cuidador", false);
//        } else {
//            $this->db->select('a.cuidador');
//        }
//        $this->db->select('e.cnpj, e.centro_custo, e.agencia_bancaria, e.conta_bancaria, e.nome_banco');
//        $this->db->select('f.nome AS solicitante, c.depto, c.mes, c.ano');
//        $this->db->select(["FORMAT(g.valor_extra1_mes{$idMes}, 2, 'de_DE') AS valor_extra1"], false);
//        $this->db->select(["FORMAT(g.valor_extra2_mes{$idMes}, 2, 'de_DE') AS valor_extra2"], false);
//        $this->db->select("g.justificativa1_mes{$idMes} AS justificativa1", false);
//        $this->db->select("g.justificativa2_mes{$idMes} AS justificativa2", false);
//        $this->db->join('ei_alocacao_escolas b', 'b.id = a.id_alocacao_escola');
//        $this->db->join('ei_alocacao c', 'c.id = b.id_alocacao');
//        $this->db->join('ei_alocados_horarios d', 'd.id_alocado = a.id');
//        if ($substituto === '2') {
//            $this->db->join('usuarios e', 'e.id = d.id_cuidador_sub2');
//        } elseif ($substituto === '1') {
//            $this->db->join('usuarios e', 'e.id = d.id_cuidador_sub1');
//        } else {
//            $this->db->join('usuarios e', 'e.id = a.id_cuidador');
//        }
//        $this->db->join('usuarios f', 'f.id = c.coordenador');
//        $this->db->join('ei_pagamento_prestador g', 'g.id_alocacao = c.id AND g.id_cuidador = a.id_cuidador', 'left');
//        $this->db->where('a.id', $idAlocado);
//
//        $data = $this->db->get('ei_alocados a')->row();
        ...
        $this->db->start_cache();
        $this->db->select('a.id, a0.escola, c.nome_banco, c.agencia_bancaria, c.conta_bancaria, b.depto, c.cnpj, c.centro_custo, d.nome AS solicitante, a1.id AS id_totalizacao ');
        $this->db->select("(CASE b2.periodo WHEN 0 THEN 'Madrugada' WHEN 1 THEN 'Manhã' WHEN 2 THEN 'Tarde' WHEN 3 THEN 'Noite' END) AS nome_periodo", false);
        if ($substituto === '2') {
            $this->db->select("GROUP_CONCAT(DISTINCT c.nome ORDER BY c.nome SEPARATOR ', ') AS cuidador", false);
            $this->db->select(["FORMAT((IF(i.valor_pagamento > 0, i.valor_pagamento, a.valor_hora_pagamento) * ((TIME_TO_SEC(a1.total_horas_sub2_mes{$idMes}) + TIME_TO_SEC(IFNULL(a1.horas_descontadas_sub2_mes{$idMes}, '00;00'))) / 3600) + IFNULL(a.valor_extra1_mes{$idMes}, 0) + IFNULL(a.valor_extra2_mes{$idMes}, 0)), 2, 'de_DE') AS valor_soma"], false);
            if ($recuperar) {
                $this->db->select(["TIME_FORMAT(ADDTIME(a1.total_horas_sub2_mes{$idMes}, IFNULL(a1.horas_descontadas_sub2_mes{$idMes}, '00;00')), '%H:%i') AS total_horas_mes"], false);
                $this->db->select(["FORMAT(SUM(DISTINCT i.valor_pagamento), 2, 'de_DE') AS valor_hora_pagamento"], false);
                $this->db->select(["FORMAT(IF(i.valor_pagamento > 0, i.valor_pagamento, a1.valor_hora_pagamento) * ((TIME_TO_SEC(a1.total_horas_sub2_mes{$idMes}) + TIME_TO_SEC(IFNULL(a1.horas_descontadas_sub2_mes{$idMes}, '00;00'))) / 3600), 2, 'de_DE') AS valor_total"], false);
            } else {
                $this->db->select(["TIME_FORMAT(IFNULL(a1.total_horas_faturadas_sub2, ADDTIME(a1.total_horas_sub2_mes{$idMes}, IFNULL(a1.horas_descontadas_sub2_mes{$idMes}, '00;00'))), '%H:%i') AS total_horas_mes"], false);
                $this->db->select(["FORMAT(IFNULL(a1.valor_pagamento_sub2, SUM(DISTINCT i.valor_pagamento)), 2, 'de_DE') AS valor_hora_pagamento"], false);
                $this->db->select(["FORMAT(IFNULL(a1.valor_total_sub2, IF(i.valor_pagamento > 0, i.valor_pagamento, a1.valor_hora_pagamento) * ((TIME_TO_SEC(a1.total_horas_sub2_mes{$idMes}) + TIME_TO_SEC(IFNULL(a1.horas_descontadas_sub2_mes{$idMes}, '00;00'))) / 3600)), 2, 'de_DE') AS valor_total"], false);
            }
        } elseif ($substituto === '1') {
            $this->db->select("GROUP_CONCAT(DISTINCT c.nome ORDER BY c.nome SEPARATOR ', ') AS cuidador", false);
            $this->db->select(["FORMAT((IF(i.valor_pagamento > 0, i.valor_pagamento, i.valor_pagamento) * ((TIME_TO_SEC(a1.total_horas_sub1_mes{$idMes}) + TIME_TO_SEC(IFNULL(a1.horas_descontadas_sub1_mes{$idMes}, '00;00'))) / 3600) + IFNULL(a.valor_extra1_mes{$idMes}, 0) + IFNULL(a.valor_extra2_mes{$idMes}, 0)), 2, 'de_DE') AS valor_soma"], false);
            if ($recuperar) {
                $this->db->select(["TIME_FORMAT(ADDTIME(a1.total_horas_sub1_mes{$idMes}, IFNULL(a1.horas_descontadas_sub1_mes{$idMes}, '00;00')), '%H:%i') AS total_horas_mes"], false);
                $this->db->select(["FORMAT(SUM(DISTINCT i.valor_pagamento), 2, 'de_DE') AS valor_hora_pagamento"], false);
                $this->db->select(["FORMAT(IF(i.valor_pagamento > 0, i.valor_pagamento, i.valor_pagamento) * ((TIME_TO_SEC(a1.total_horas_sub1_mes{$idMes}) + TIME_TO_SEC(IFNULL(a1.horas_descontadas_sub1_mes{$idMes}, '00;00'))) / 3600), 2, 'de_DE') AS valor_total"], false);
            } else {
                $this->db->select(["TIME_FORMAT(IFNULL(a1.total_horas_faturadas_sub1, ADDTIME(a1.total_horas_sub1_mes{$idMes}, IFNULL(a1.horas_descontadas_sub1_mes{$idMes}, '00;00'))), '%H:%i') AS total_horas_mes"], false);
                $this->db->select(["FORMAT(IFNULL(a1.valor_pagamento_sub1 ,SUM(DISTINCT i.valor_pagamento)), 2, 'de_DE') AS valor_hora_pagamento"], false);
                $this->db->select(["FORMAT(IFNULL(a1.valor_total_sub1, IF(i.valor_pagamento > 0, i.valor_pagamento, i.valor_pagamento) * ((TIME_TO_SEC(a1.total_horas_sub1_mes{$idMes}) + TIME_TO_SEC(IFNULL(a1.horas_descontadas_sub1_mes{$idMes}, '00;00'))) / 3600)), 2, 'de_DE') AS valor_total"], false);
            }
        } else {
            $this->db->select('a.cuidador');
            if ($recuperar) {
                $this->db->select(["TIME_FORMAT(SEC_TO_TIME(TIME_TO_SEC(e2.horas_mensais_custo) + TIME_TO_SEC(a1.horas_descontadas_mes{$idMes})), '%H:%i') AS total_horas_mes"], false);
                $this->db->select(["FORMAT(e2.valor_hora_operacional, 2, 'de_DE') AS valor_hora_pagamento"], false);
                $this->db->select(["FORMAT(e2.valor_hora_operacional * (IFNULL(TIME_TO_SEC(e2.horas_mensais_custo) + TIME_TO_SEC(a1.horas_descontadas_mes{$idMes}), 0) / 3600), 2, 'de_DE') AS valor_total"], false);
                $this->db->select(["FORMAT(e2.valor_hora_operacional * (IFNULL(TIME_TO_SEC(e2.horas_mensais_custo), 0) / 3600) + IFNULL(j.valor_extra1_mes{$idMes}, 0) + IFNULL(j.valor_extra2_mes{$idMes}, 0), 2, 'de_DE') AS valor_soma"], false);
            } else {
                $this->db->select(["TIME_FORMAT(IFNULL(a1.total_horas_faturadas_mes{$idMes}, SEC_TO_TIME(TIME_TO_SEC(e2.horas_mensais_custo) + TIME_TO_SEC(a1.horas_descontadas_mes{$idMes}))), '%H:%i') AS total_horas_mes"], false);
//                $this->db->select(["TIME_FORMAT(IFNULL(a1.total_horas_faturadas_mes{$idMes}, SEC_TO_TIME(TIME_TO_SEC(e2.horas_mensais_custo) + IFNULL(a1.total_dias_mes{$idMes}, 0))), '%H:%i') AS total_horas_mes"], false);
                $this->db->select(["FORMAT(IFNULL(a1.valor_pagamento_mes{$idMes}, e2.valor_hora_operacional), 2, 'de_DE') AS valor_hora_pagamento"], false);
                $this->db->select(["FORMAT(IFNULL(a1.valor_total_mes{$idMes}, e2.valor_hora_operacional * (IFNULL(TIME_TO_SEC(e2.horas_mensais_custo) + TIME_TO_SEC(a1.horas_descontadas_mes{$idMes}), 0) / 3600)), 2, 'de_DE') AS valor_total"], false);
                $this->db->select(["FORMAT(IFNULL(a1.valor_total_mes{$idMes}, e2.valor_hora_operacional * (IFNULL(TIME_TO_SEC(e2.horas_mensais_custo) + TIME_TO_SEC(a1.horas_descontadas_mes{$idMes}), 0) / 3600)) + IFNULL(j.valor_extra1_mes{$idMes}, 0) + IFNULL(j.valor_extra2_mes{$idMes}, 0), 2, 'de_DE') AS valor_soma"], false);
            }
        }
        $this->db->select(["FORMAT(j.valor_extra1_mes{$idMes}, 2, 'de_DE') AS valor_extra1"], false);
        $this->db->select(["FORMAT(j.valor_extra2_mes{$idMes}, 2, 'de_DE') AS valor_extra2"], false);
        $this->db->select("j.justificativa1_mes{$idMes} AS justificativa1", false);
        $this->db->select("j.justificativa2_mes{$idMes} AS justificativa2", false);
        $this->db->join('ei_alocacao_escolas a0', 'a0.id = a.id_alocacao_escola');
        $this->db->join('ei_alocacao b', 'b.id = a0.id_alocacao');
        $this->db->join('ei_alocados_horarios b2', 'b2.id_alocado = a.id');
        if ($substituto === '2') {
            $this->db->join('usuarios c', 'c.id = b2.id_cuidador_sub2');
        } elseif ($substituto === '1') {
            $this->db->join('usuarios c', 'c.id = b2.id_cuidador_sub1');
        } else {
            $this->db->join('usuarios c', 'c.id = a.id_cuidador');
        }
        $this->db->join('usuarios d', 'd.id = b.coordenador');
        $this->db->join('ei_alocados_totalizacao a1', 'a1.id_alocado = a.id AND a1.periodo = b2.periodo', 'left');
        $this->db->join('ei_ordem_servico_profissionais e', 'e.id = a.id_os_profissional', 'left');
//        $this->db->join('ei_ordem_servico_horarios e2', 'e2.id_os_profissional = e.id AND e2.id = b2.id_os_horario AND (e2.data_inicio_contrato IS NOT NULL OR e2.data_termino_contrato IS NOT NULL) AND (e2.valor_hora_operacional IS NOT NULL AND e2.horas_mensais_custo IS NOT NULL)', 'left');
        $this->db->join('ei_ordem_servico_horarios e2', 'e2.id_os_profissional = e.id AND e2.id = b2.id_os_horario', 'left');
        $this->db->join('empresa_cargos f', 'f.nome = c.cargo', 'left');
        $this->db->join('empresa_funcoes g', 'g.nome = c.funcao', 'left');
        $this->db->join('ei_contratos h', 'h.contrato = a0.contrato', 'left');
        $this->db->join('ei_valores_faturamento i', 'i.id_contrato = h.id AND i.ano = b.ano AND i.semestre = b.semestre AND i.id_cargo = f.id AND i.id_funcao = g.id', 'left');
        $this->db->join('ei_pagamento_prestador j', 'j.id_alocacao = b.id AND j.id_cuidador = a.id_cuidador', 'left');
        $this->db->where('a.id', $idAlocado);
        $this->db->group_by('a.id_cuidador');
        $this->db->stop_cache();
        $data = $this->db->get('ei_alocados a')->row();


        $this->db->group_by('a0.escola, b2.funcao, b2.periodo');
        $rows = $this->db->get('ei_alocados a')->result();
        $this->db->flush_cache();


        $servicos = array();
        $soma = floatval(str_replace(['.', ','], ['', '.'], $data->valor_extra1));
        $soma += floatval(str_replace(['.', ','], ['', '.'], $data->valor_extra2));
        foreach ($rows as $row) {
            $servicos[] = array(
                'id' => $row->id_totalizacao,
                'escola' => $row->escola,
                'periodo' => $row->nome_periodo,
                'qtdeHoras' => $row->total_horas_mes,
                'valorCustoProfissional' => $row->valor_hora_pagamento,
                'total' => $row->valor_total
            );


            $soma += floatval(str_replace(['.', ','], ['', '.'], $row->valor_total));
        }
        ...
        $planilha = array(
            ...
            'prestador' => $data->cuidador,
            'cnpj' => $data->cnpj,
            'departamento' => $data->depto,
            'centroCusto' => $data->centro_custo,
            'agencia' => $data->agencia_bancaria,
            'conta' => $data->conta_bancaria,
            'banco' => $data->nome_banco,
            'mesAno' => ucfirst($this->calendar->get_month_name($mes)) . '/' . $ano,
            'justificativa1' => $data->justificativa1,
            'valorExtra1' => $data->valor_extra1,
            'justificativa2' => $data->justificativa2,
            'valorExtra2' => $data->valor_extra2,
    ...
    public function planilhaPagamentoPrestador($idHorario, $mes, $ano, $is_pdf = false, $recuperar = false)
    {
        ...
        if ($substituto === '2') {
            ...
            $this->db->select(['NULL AS valor_hora_pagamento'], false);
            ...
            if ($recuperar) {
                ...
                $this->db->select(["FORMAT(e.valor_hora_operacional, 2, 'de_DE') AS valor_hora_pagamento"], false);
                $this->db->select(["FORMAT(e.valor_hora_operacional * ((IFNULL(TIME_TO_SEC(e.horas_mensais_custo), 0) + IFNULL(TIME_TO_SEC(d.horas_descontadas_mes{$idMes}), 0)) / 3600), 2, 'de_DE') AS valor_total"], false);
            } else {
                ...
                $this->db->select(["FORMAT(IFNULL(d.valor_pagamento_mes{$idMes}, e.valor_hora_operacional), 2, 'de_DE') AS valor_hora_pagamento"], false);
                $this->db->select(["FORMAT(IFNULL(d.valor_total_mes{$idMes}, e.valor_hora_operacional * ((IFNULL(TIME_TO_SEC(e.horas_mensais_custo), 0) + IFNULL(TIME_TO_SEC(d.horas_descontadas_mes{$idMes}), 0)) / 3600)), 2, 'de_DE') AS valor_total"], false);
            }
        }
        $this->db->join('ei_alocados b', 'b.id = a.id_alocado');
        $this->db->join('ei_alocacao_escolas c', 'c.id = b.id_alocacao_escola');
        ...
        foreach ($totalizacoes as $totalizacao) {
            $servicos[] = array(
                ...
                'valorCustoProfissional' => $totalizacao->valor_hora_pagamento,
...
//        $data = $this->db->get('ei_alocados a')->row();
        ...
        if ($substituto === '2') {
            ...
            $this->db->select('a.cuidador');
            if ($recuperar) {
                $this->db->select(["TIME_FORMAT(SEC_TO_TIME(TIME_TO_SEC(e2.horas_mensais_custo) + IFNULL(TIME_TO_SEC(a1.horas_descontadas_mes{$idMes}), 0)), '%H:%i') AS total_horas_mes"], false);
                $this->db->select(["FORMAT(e2.valor_hora_operacional, 2, 'de_DE') AS valor_hora_pagamento"], false);
                $this->db->select(["FORMAT(e2.valor_hora_operacional * (IFNULL(TIME_TO_SEC(e2.horas_mensais_custo) + TIME_TO_SEC(a1.horas_descontadas_mes{$idMes}), 0) / 3600), 2, 'de_DE') AS valor_total"], false);
                $this->db->select(["FORMAT(e2.valor_hora_operacional * (IFNULL(TIME_TO_SEC(e2.horas_mensais_custo), 0) / 3600) + IFNULL(j.valor_extra1_mes{$idMes}, 0) + IFNULL(j.valor_extra2_mes{$idMes}, 0), 2, 'de_DE') AS valor_soma"], false);
            } else {
