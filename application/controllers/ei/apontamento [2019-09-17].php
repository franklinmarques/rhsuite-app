...
//        }
    ...
    public function ajaxSaveFaturamentoConsolidado()
    {
        ...
        $dataObs = ["observacoes_mes{$mes}" => strlen($obs) > 0 ? $obs : null];
...
//        $this->db->select("SUM(CASE WHEN e.status IN ('FE', 'EM', 'RE') THEN 1 ELSE 0 END) + SUM(IFNULL(d.desconto_sub1, 0)) + SUM(IFNULL(d.desconto_sub2, 0)) AS total_dias_descontos", false);
    ...
    public function planilhaFaturamento($idAlocado, $idMes, $periodo, $is_pdf = false, $recuperar = false)
    {
        ...
        $mes = str_pad($idMes + ($supervisor->semestre > 1 ? 6 : 0), 2, '0', STR_PAD_LEFT);
...
//            $totalHoras += preg_match('/^\d{2,}:\d{2}:\d{2}$/', $alocado->total_segundos) ? timeToSec($alocado->total_segundos) : $alocado->total_segundos;
    ...
    public function planilhaPagamentoPrestador($idHorario, $idMes, $ano, $is_pdf = false, $recuperar = false)
    {
        ...
        $mes = str_pad($idMes + (intval($alocacao->semestre) > 1 ? 6 : 0), 2, '0', STR_PAD_LEFT);
...
//        $this->db->select(["TIME_FORMAT(SEC_TO_TIME(TIME_TO_SEC(e.horas_mensais_custo) + IFNULL(TIME_TO_SEC(d.horas_descontadas_mes{$idMes}), 0)), '%H:%i') AS total_horas_mes"], false);
        ...
        $this->db->select('a3.nome AS solicitante, a.depto', false);
        $this->db->select("e.valor_extra1_mes{$idMes} AS valor_extra1", false);
        $this->db->select("e.valor_extra2_mes{$idMes} AS valor_extra2", false);
        $this->db->select("FORMAT(IFNULL(c21.valor_hora_operacional, c2.valor_hora_operacional), 2, 'de_DE') AS valor_hora_operacional", false);
        $this->db->select("FORMAT(c6.valor_pagamento, 2, 'de_DE') AS valor_pagamento", false);
        $this->db->select("FORMAT(c6.valor_pagamento2, 2, 'de_DE') AS valor_pagamento2", false);
        ...
        $this->db->select("IFNULL(e.tipo_pagamento_mes{$idMes}, 1) AS tipo_pagamento", false);
        ...
        $this->db->join('ei_diretorias a2', 'a2.id = a.id_diretoria');
        $this->db->join('usuarios a3', 'a3.id = a2.id_coordenador');
        $this->db->join('ei_alocacao_escolas b', 'b.id_alocacao = a.id');
        $this->db->join('ei_alocados c', 'c.id_alocacao_escola = b.id');
        $this->db->join('ei_ordem_servico_profissionais c2', 'c2.id = c.id_os_profissional');
        $this->db->join('ei_ordem_servico_escolas c3', 'c3.id = c2.id_ordem_servico_escola');
        $this->db->join('ei_ordem_servico c4', 'c4.id = c3.id_ordem_servico');
        $this->db->join('ei_contratos c5', 'c5.id = c4.id_contrato');
        $this->db->join('ei_valores_faturamento c6', 'c6.id_contrato = c5.id AND c6.ano = c4.ano AND c6.semestre = c4.semestre AND c6.id_funcao = c2.id_funcao', 'left');
        $this->db->join('usuarios d', 'd.id = c.id_cuidador');
        $this->db->join('ei_ordem_servico_horarios c21', 'c21.id_os_profissional = c2.id', 'left');
...
//print_r($pagamentoPrestador->buu);exit;
        ...
        if ($substituto) {
            ...
            if ($recuperar) {
//                $this->db->select(["TIME_FORMAT(  SEC_TO_TIME(SUM(IFNULL(TIME_TO_SEC(a.total_horas_mes{$idMes}), 0)))  , '%H:%i') AS total_horas_mes"], false);
                $this->db->select(["TIME_FORMAT(SEC_TO_TIME(TIME_TO_SEC(e.horas_mensais_custo) + IFNULL(TIME_TO_SEC(d.horas_descontadas_mes{$idMes}), 0)), '%H:%i') AS total_horas_mes"], false);
...
//                $this->db->select(["TIME_FORMAT(IFNULL(d.total_horas_faturadas_mes{$idMes}, SEC_TO_TIME(SUM(TIME_TO_SEC(e.horas_mensais_custo) + IFNULL(TIME_TO_SEC(d.horas_descontadas_mes{$idMes}), 0))), '%H:%i') AS total_horas_mes"], false);
                ...
                $this->db->select(["IFNULL(d.valor_pagamento_mes{$idMes}, IF(e.valor_hora_operacional > 0, e.valor_hora_operacional, j.valor_pagamento)) AS valor_hora_operacional"], false);
                $this->db->select(["IFNULL(d.valor_total_mes{$idMes}, IF(e.valor_hora_operacional > 0, e.valor_hora_operacional, j.valor_pagamento) * ((IFNULL(TIME_TO_SEC(e.horas_mensais_custo), 0) + IFNULL(TIME_TO_SEC(d.horas_descontadas_mes{$idMes}), 0)) / 3600)) AS valor_total"], false);
            }
        }
        $this->db->join('ei_alocados b', 'b.id = a.id_alocado');
        $this->db->join('ei_alocacao_escolas c', 'c.id = b.id_alocacao_escola');
...
//print_r(number_format(1222.227, 2, ',', '.'));exit;
...
//            'pagamento_ordem_servico' => number_format(round($totalizacoes[0]->valor_hora_operacional, 2, PHP_ROUND_HALF_DOWN), 2, ',', '.'),
