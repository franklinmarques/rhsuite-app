...
//            if ($row->status === '0' and $row->filhas_finalizadas === '1') {
	...
	private function notificar(array $rows)
	{
		...
		$usuarios = $this->db->select('id, nome, email')
		...
		foreach ($usuarios as $usuario) {
			...
			$nomeDestinatario[$usuario->id] = $usuario->nome;
		}
		...
		foreach ($rows as $row) {
			...
			/*$msg = $data['nome'] . '<br>';
			$msg .= '<h5>Atribuição de nova atividade<h5>';
			$msg .= '<h3>Caro colabrador(a),</h3>
                     <p>Uma nova tarefa lhe foi atribuída. Por gentileza, verifique a sua lista de atividades.</p>';
			$msg .= "<fieldset>
                         <legend>Dados da atividade</legend>
                             <strong>Descritivo:</strong> {$data['atividade']}<br/>
                             <strong>Quem lhe atribuiu:</strong> {$data['nome']} [<a href=\"mailto:{$data['email']}\">{$data['email']}</a>]<br/>
                             <strong>Data limite:</strong> {$data['dataLimite']}
                      </fieldset>";*/

			$this->email
				->from('contato@rhsuite.com.br', 'RhSuite')
				->to($emails[$row['id_usuario']])
				->subject($nomeDestinatario[$row['id_usuario']] . ' - Atribuição de nova atividade')
