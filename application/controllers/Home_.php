...
class Home extends CI_Controller
{
    ...
    public function editarempresa_json()
    {
        ...
        $verificaEmail = $this->db->query("SELECT * FROM usuarios WHERE email = ? AND id <> ?", array($data['email'], $this->uri->segment(3)));
        if ($verificaEmail->num_rows > 0)
    ...
    public function getInfoQuestao($id)
    {

        $res = $this->db->query("SELECT * FROM atividadesperguntas WHERE id = " . $id);
    ...
    public function novapaginacurso()
    {
        ...
        $curso = $this->db->query("SELECT * FROM cursos WHERE id = ?", array($this->uri->segment(3)))->row(0);
        $paginaRes = $this->db->query("SELECT * FROM paginas WHERE curso = ? ORDER BY ordem DESC LIMIT 1", array($curso->id));
        $pagina = $paginaRes->result()[0];
        ...
        $res = $this->db->query("SELECT id_atividadeperguntas FROM bibliotecaquestoes 
																			WHERE bibliotecaquestoes.id_usuario = {$this->session->userdata('empresa')}");

        $questoesDaBiblioteca = $res->result();
        foreach ($questoesDaBiblioteca as $k => $questao) {
            $perguntas[] = $this->getInfoQuestao($questao->id_atividadeperguntas);
        }

        $data['query'] = $perguntas;
        ...
        if ($data['modulo'] == "ckeditor") {
            ...
            if (empty($_POST['videoyoutube'])) {
                if (!empty($_FILES['arquivoVideo'])) {
                    ...
                    if ($this->upload->do_upload('arquivoVideo')) {
                        $arquivo = $this->upload->data();
                        $data['arquivoVideo'] = $arquivo['file_name'];
            ...
            foreach ($perguntas as $pid => $pergunta) {
                ...
                $quiz[$pid] = array(
                    'pid' => $pid,
                    'pergunta' => $pergunta,
                    'tipoquestao' => $_POST['tipoquestao'][$pid],
                    'respostacorreta' => $_POST['respostacorreta'][$pid],
                    'respostaerrada' => $_POST['respostaerrada'][$pid],
                    'quantidade' => count($_POST['alternativas'][$pid])
                );
            ...
            if ($this->db->query($this->db->insert_string('paginas', $data))) {
                ...
                //#tirar
                foreach ($quiz as $row) {
                    //CASO A PERGUNTA SEJA NOVA - NAO TEM NA BIBLIOTECA NEM EM OUTRAS PAGINAS
                    if ($verificaPergunta == 0 && $verificaPerguntaBiblioteca == 0) {
                        $dataQuizPerguntas['pagina'] = $id_pagina;
                        $dataQuizPerguntas['pergunta'] = $row['pergunta'];
                        $dataQuizPerguntas['tipo'] = $row['tipoquestao'];
                        $dataQuizPerguntas['respostacorreta'] = $row['respostacorreta'];
                        $dataQuizPerguntas['respostaerrada'] = $row['respostaerrada'];
                        ...
                        if ($this->db->query($this->db->insert_string('atividadesperguntas', $dataQuizPerguntas))) {
                            $id_atividadeperguntas = $this->db->insert_id();

                            foreach ($row['perguntas'] as $row_) {
                                $dataQuizAlternativas['quiz'] = $id_atividadeperguntas;
                                $dataQuizAlternativas['alternativa'] = $row_['alternativa'];
                                $dataQuizAlternativas['correta'] = $row_['correta'];
                            ...
                            if (!empty($_POST['adicionar-biblioteca'])) {
                                $dataBibliotecaQuestoes['id_atividadeperguntas'] = $id_atividadeperguntas;
                                $dataBibliotecaQuestoes['id_pagina'] = $id_pagina;
                                $dataBibliotecaQuestoes['id_usuario'] = $this->session->userdata('id');

                                $this->db->query($this->db->insert_string('bibliotecaquestoes', $dataBibliotecaQuestoes));
                            }
                        }
                        //CASO A PERGUNTA SEJA DA PAGINA E VENHA DA BIBLIOTECA
                    } else if ($verificaPergunta > 0 && $verificaPerguntaBiblioteca > 0) {
                        //echo "EDIÇÃO DE PAGINA**";
                        ...
                        //CASO A PERGUNTA EXISTA NA BIBLIOTECA MAS NAO ESTÁ NA PAGINA	
                    } else if ($verificaPergunta == 0 && $verificaPerguntaBiblioteca > 0) {
                        //echo "INSERIR PERGUNTA DA BIBLIOTECA**";
        ...
        if ($data['modulo'] != "atividades") {
            if ($this->db->query($this->db->insert_string('paginas', $data))) {
                ...
                /*
                  ...
                  foreach ($id_pergunta as $row) {
                  $databibliotecaquestoes = array( 'id_pagina' => $idPagina, 'id_atividadeperguntas' =>$row, 'id_usuario' => $this->session->userdata('empresa') );
                  $this->db->query($this->db->insert_string('bibliotecaquestoes', $databibliotecaquestoes));
                  }
                  }
                  }
    ...
    public function editarpaginacurso()
    {
        ...
        //voltar
        $data['atividadesperguntas'] = $this->db->query("SELECT 
																	atividadesalternativas.id as id_atividadesalternativas,
																	atividadesalternativas.correta, 
																	atividadesalternativas.alternativa,
																	atividadesperguntas.id, 
																	atividadesperguntas.pergunta, 
																	atividadesperguntas.tipo,
																	atividadesperguntas.respostacorreta, 
																	atividadesperguntas.respostaerrada
																		
																				FROM bibliotecaquestoes 
																					LEFT JOIN atividadesperguntas on atividadesperguntas.id = bibliotecaquestoes.id_atividadeperguntas 
																					LEFT JOIN atividadesalternativas ON atividadesperguntas.id = atividadesalternativas.quiz
																						WHERE bibliotecaquestoes.id_pagina = ? 
																						GROUP BY atividadesperguntas.id ORDER BY id ASC", array($pagina->id));

        $data['categoria'] = $this->db->query("SELECT * FROM categoria ORDER BY id ASC");
        $data['bibliotecaDeQuestoes'] = $this->db->query(" 
															SELECT 
																atividadesperguntas.id,
																atividadesperguntas.pergunta
																	FROM
																 atividadesperguntas 
																	WHERE atividadesperguntas.pagina = ? ORDER BY id ASC", array($pagina->id));



        $resQuestoesParaAdicionar = $this->db->query("SELECT 
															atividadesperguntas.id,
															atividadesperguntas.pergunta
																FROM bibliotecaquestoes
																	RIGHT JOIN atividadesperguntas ON atividadesperguntas.id = bibliotecaquestoes.id_atividadeperguntas
																				WHERE 
																					bibliotecaquestoes.id_usuario = {$this->session->userdata('empresa')}
																						AND bibliotecaquestoes.id_pagina <> $pagina->id
																						ORDER BY atividadesperguntas.pergunta ASC");

        $data['questoesParaAdicioanr'] = $resQuestoesParaAdicionar->result();
        $this->load->view('editarpaginacurso', $data);
    }

    public function removerQuestaoBiblioteca()
    {
        //header('Content-type: text/json');
        ...
        if ($this->db->delete('bibliotecaquestoes', array('id_atividadeperguntas' => $_POST['id']))) {

            $this->db->delete('atividadesperguntas', array('id' => $_POST['id']));
            $this->db->delete('atividadesalternativas', array('quiz' => $_POST['id']));

            echo json_encode(array('success' => true, 'message' => 'excluido com sucesso'));
        } else {
            echo json_encode(array('success' => false, 'message' => 'não pode ser deletado'));
        }
    }
    ...
    public function editarpaginacurso_json()
    {
        ...
        $pagina = $this->db->query("SELECT * FROM paginas WHERE id = " . $this->uri->segment(3))->row(0);
        $curso = $this->db->query("SELECT * FROM cursos WHERE id = ?", array($pagina->curso))->row(0);
        ...
        if ($data['modulo'] == "ckeditor") {
            ...
            $aid_ = array();
            $quiz = array();
            $id_pagina = $this->uri->segment(3);

            $perguntas = $_POST['perguntas'];

            foreach ($perguntas as $pid => $pergunta) {
                $pid_[] = $pid;
                $quiz[$pid] = array('pid' => $pid,
                    'pergunta' => $pergunta,
                    'tipoquestao' => $_POST['tipoquestao'][$pid],
                    'respostacorreta' => $_POST['respostacorreta'][$pid],
                    'respostaerrada' => $_POST['respostaerrada'][$pid],
            ...
            $quizPerguntas = $this->db->query("SELECT * FROM atividadesperguntas WHERE pagina = ? ORDER BY id ASC", array($id_pagina));
            ...
            foreach ($quiz as $row) {
                $id_pergunta = "'" . $row['pid'] . "'";
                //echo "SELECT * FROM atividadesperguntas WHERE id = '".$row['pid']."' AND pagina = " .$id_pagina;
                $verificaPergunta = $this->db->query("SELECT * FROM atividadesperguntas WHERE id = '" . $row['pid'] . "' AND pagina = ?", array($id_pagina))->num_rows;
                $verificaPerguntaBiblioteca = $this->db->query("SELECT * FROM bibliotecaquestoes WHERE id_atividadeperguntas = '" . $row['pid'] . "'")->num_rows;
    ...
    public function getQuestaoPorId()
    {
        header('Content-type: text/json');

        $result = $this->db->query("SELECT * FROM atividadesperguntas WHERE id = ?", array($_POST['id']));

        foreach ($result->result() as $k => $questao) {
            $questoes['questoes']['id'] = $questao->id;
            $questoes['questoes']['pagina'] = $questao->pagina;
            $questoes['questoes']['tipo'] = $questao->tipo;
            $questoes['questoes']['pergunta'] = $questao->pergunta;
            $questoes['questoes']['respostacorreta'] = $questao->respostacorreta;
            $questoes['questoes']['respostaerrada'] = $questao->respostaerrada;
            $questoes['questoes']['copia_de'] = $questao->copia_de;

            $alt = $this->db->query("SELECT * FROM atividadesalternativas WHERE quiz = ?", array($_POST['id']));
            $questoes['questoes']['alternativas'] = $alt->result();

            $json = json_encode("SELECT * FROM atividadesalternativas WHERE quiz = " . $_POST['id']);
            $file = "./application/logs/log.txt";
    ...
    public function excluirpaginacurso()
    {
        ...
        $this->db->where('id_pagina', $pagina->id)->delete('bibliotecaquestoes');
    ...
    public function solicitarcurso_json()
    {
        ...
        if ($this->session->userdata('tipo') == "empresa") {
            ...
            foreach ($administradores->result() as $row) {
                $nome = $row->nome;
                $destino = array($row->email, 'marcosfortes@peoplenetineducation.com.br', 'mhffortes@hotmail');
                $assunto = "Peoplenet Corp - Solicitação de Treinamento";
                $mensagem = "<center>
					<h1>Peoplenet Corp - Solicitação de Treinamento</h1>
					</center>
					<hr />
					<p>Prezado(a) {$nome},</p>
					<p>Foi solicitado um curso, segue abaixo os dados da empresa e do curso solicitado.</p>
					<p><strong>Requisitante:</strong> {$this->session->userdata('nome')} - {$this->session->userdata('email')}</p>
					<p><strong>Treinamento:</strong> {$curso->curso}</p>";
                ...
                foreach ($destino as $emails) {
                    $sucesso = mail($emails, $assunto, $mensagem, $headers);
                }
            ...
            //verificando se é funcionario e se este funcionario tem este curso
            /*
            ...
            $mensagem = "<center>
                ...
                <hr />
                <p>Prezado(a) {$nome},</p>
                <p>Foi solicitado um treinamento, segue abaixo os dados do funcionário e do curso solicitado.</p>
                <p><strong>Requisitante:</strong> {$this->session->userdata('nome')} - {$this->session->userdata('email')}</p>
                <p><strong>Treinamento:</strong> {$curso->curso}</p>";
            ...
            $sucesso = mail($destino, $assunto, $mensagem, $headers);
            if ($enviaremail) {
                $sucesso = 1;
            }
        }
    ...
    public function novocursofuncionario()
    {
        ...
        $data['cursos'] = $this->db->query("SELECT * FROM cursos WHERE usuario = ? AND id NOT IN
                                            ...
                                            id IN (SELECT curso FROM usuarioscursos WHERE usuario = ? AND usuario <> ?)", array($this->session->userdata('empresa'), $this->uri->segment(3),
            $this->session->userdata('empresa'), $this->uri->segment(3)));
    ...
    public function bibliotecaDeQuestoes()
    {
        $atividadesperguntas = $this->db->query("SELECT * FROM atividadesperguntas");

        $data['row'] = $atividadesperguntas;

        $this->load->view('bibliotecaDeQuestoes', $data);
    }

    public function getbibliotecaDeQuestoes()
    {
        if (!in_array($this->session->userdata('tipo'), array("administrador", "empresa")))
            redirect(base_url('home'));
        ...
        $atividadesperguntas = $this->db->query("SELECT * FROM atividadesperguntas ")->row(0);

        $query = $_POST['busca'];

        $qryWHERE = 'WHERE pergunta = ? ';
        $dataWHERE[] = '';

        if (!empty($query)) {
            $qryWHERE .= 'AND (pergunta LIKE ?)';
            $dataWHERE[] = "%{$query}%";
        }
        ...
        $config['base_url'] = base_url('home/getbibliotecaDeQuestoes/' . $atividadesperguntas->id);

        $config['total_rows'] = $this->db->query("SELECT * FROM atividadesperguntas")->num_rows;
        $config['per_page'] = 9999;
        $config['uri_segment'] = 4;
        ...
        $data['total'] = $this->db->query("SELECT * FROM atividadesperguntas ");
        $data['busca'] = "busca={$query}&{$this->security->get_csrf_token_name()}={$this->security->get_csrf_hash()}";
        $data['query'] = $this->db->query("SELECT * FROM atividadesperguntas ORDER BY id ASC ");


        $this->load->view('getbibliotecaDeQuestoes', $data);
    }
