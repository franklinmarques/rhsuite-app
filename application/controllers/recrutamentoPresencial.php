...
class RecrutamentoPresencial extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('recrutamentoPresencial', $data);
    }
    ...
    public function ajax_list($candidato = '')
    {
        ...
        foreach ($list as $recrutamento) {
            ...
            $row[] = $recrutamento->tipo_vaga;
            $row[] = $recrutamento->requisitante;
            $row[] = $recrutamento->previsao_inicio;
    ...
    public function relatorio($id_teste, $pdf = false)
    {
        ...
        $sql = "SELECT a.numero AS nome, 
                       ...
                       IFNULL(h.nome, a.requisitante_externo) AS requisitante, 
                       b.nome AS cargo, 
                       d.nome AS candidato,
                       g.nota
                FROM requisicoes_pessoal a
                INNER JOIN empresa_cargos b ON
                           b.id = a.id_cargo
                INNER JOIN requisicoes_pessoal_candidatos c ON
                           c.id_requisicao = a.id
                INNER JOIN recrutamento_usuarios d ON
                           d.id = c.id_usuario
                INNER JOIN requisicoes_pessoal_testes e ON 
                           e.id_candidato = c.id
                INNER JOIN recrutamento_modelos f ON 
                           f.id = e.id_modelo
                LEFT JOIN usuarios h ON
                           h.id = a.requisitante_interno
                LEFT JOIN requisicoes_pessoal_resultado g ON
                          g.id_teste = f.id
                WHERE e.id = {$id_teste}";
        ...
        if ($data['teste']->tipo === 'D' || $data['teste']->tipo === 'I') {
            ...
            $this->db->join('requisicoes_pessoal_resultado d', 'd.id_teste = a.id', 'left');
            $this->db->where('a.id', $data['teste']->id_teste);
            $pergunta = $this->db->get('requisicoes_pessoal_testes a')->row();
            ...
            $this->db->join('requisicoes_pessoal_resultado c', "c.id_pergunta = a.id AND c.id_teste = '{$data['teste']->id_teste}'", 'left');
...
//           exit;
            ...
            $this->db->join('requisicoes_pessoal_resultado d', 'd.id_pergunta = c.id AND d.id_teste = a.id', 'left');
            $this->db->where('a.id', $data['teste']->id_teste);
            $this->db->where('a.id_modelo', $data['teste']->id_modelo);
            $perguntas = $this->db->get('requisicoes_pessoal_testes a')->result();
        ...
        if ($pdf) {
            ...
            if ($data['teste']->tipo == 'C') {
                $this->load->view('recrutamentoPresencial_relatorioEneagrama', $data);
            } elseif ($data['teste']->tipo == 'E') {
                $this->load->view('recrutamentPresencial_relatorioEntrevista', $data);
            } else {
                $this->load->view('recrutamentoPresencial_relatorio', $data);
            }
        }
    }
    ...
    public function pdfRelatorio()
    {
        ...
        $sql = "SELECT a.numero AS processo,
                       d.nome
                FROM requisicoes_pessoal a
                INNER JOIN empresa_cargos b ON
                           b.id = a.id_cargo
