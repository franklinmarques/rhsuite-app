...
class Recrutamento_testes extends MY_Controller
{
    ...
    private function iniciar($tipo, $id)
    {
        ...
        $sql = "SELECT f.id,
                       ...
                       (CASE WHEN f.data_acesso is null
                        ...
                        CONCAT(a.nome, ' - ', c.cargo, ': ', g.nome) AS titulo
                FROM recrutamento a
                ...
                INNER JOIN recrutamento_modelos g ON 
                           g.id = f.id_modelo
                LEFT JOIN recrutamento_resultado h ON
                          h.id_teste = f.id
        ...
        $update = $this->db->update_string('recrutamento_testes', $setData, $where);
        ...
        $this->load->view('recrutamento_exame', $data);
    }
    ...
    public function texto_exemplo($id)
    {
        $sql = "SELECT a.pergunta, b.tipo 
                ...
                INNER JOIN recrutamento_modelos b ON 
                           b.id = a.id_modelo 
                INNER JOIN recrutamento_testes c ON 
                           c.id_modelo = b.id 
                WHERE c.id = {$id} AND 
        ...
        $sql = "SELECT s.id, 
                ...
                FROM (SELECT f.id, 
                      ...
                      INNER JOIN empresa_cargos c ON
                                 c.id = a.id_cargo
                      INNER JOIN empresa_funcoes c2 ON
                                 c2.id = a.id_funcao
...
//            $row[] = $recrutamento->processo;
    ...
    public function finalizar($id_teste)
    {
        ...
        $this->db->delete('recrutamento_resultado');
        ...
        $this->db->insert_batch('recrutamento_resultado', $data);


        if ($this->db->trans_status() !== false) {
            $this->db->update('recrutamento_testes', array('data_envio' => $data_envio), array('id' => $id_teste));
        }
    ...
    public function avaliar()
    {
        ...
        $this->db->join('recrutamento_resultado d', 'a.id = d.id_teste AND d.id_pergunta = c.id', 'left');
        $this->db->where('a.id', $id_teste);
        $this->db->where_in('c.id', array_keys($post));
        $rows = $this->db->get('recrutamento_testes a')->result();
        ...
        if ($data) {
            $this->db->update_batch('recrutamento_resultado', $data, 'id');
        }
        if ($data2) {
            $this->db->insert_batch('recrutamento_resultado', $data2);
        }
