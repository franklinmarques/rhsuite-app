...
PIS (2)';
	...
	public function ajax_edit()
	{
		...
		$indicados = $this->db
			->select('a.id AS id_indicado, a.nome AS nome_indicado')
			->select('a.nome_mae AS nome_mae_indicado, a.nome_pai AS nome_pai_indicado')
			->select(["DATE_FORMAT(a.data_nascimento, '%d/%m/%Y') AS data_nascimento_indicado"], false)
			->select('rg AS rg_indicado, rg_orgao_emissor AS rg_orgao_emissor_indicado')
			->select(["DATE_FORMAT(a.rg_data_emissao, '%d/%m/%Y') AS rg_data_emissao_indicado"], false)
			->select('cpf AS cpf_indicado, pis AS pis_indicado')
			->join('requisicoes_pessoal_candidatos b', 'b.id_usuario = a.id')
			->where('b.aprovado_indicacao', 1)
			->get('recrutamento_usuarios a')
			->result_array();

		echo json_encode(['input' => $input, 'data' => $data, 'indicados' => $indicados]);
	}
	...
	public function ajax_add()
	{
		...
		if ($this->db->trans_status() == false) {
			exit(json_encode(['erro' => 'Não foi possível salvar a Requisição de Pessoal.']));
		}
	...
	//==========================================================================
	private function salvarIndicados($rows = [])
	{
		...
		$indicadosAExcluir = $this->db
			...
			->where('a.id_requisicao', $rows['id'])
			->where('b.fonte_contratacao', 'Indicação')
			->where('a.aprovado_indicacao', 1)
			->where_not_in('b.id', $rows['id_indicado'] + [0])
			->get('requisicoes_pessoal_candidatos')
			->result();

		if ($indicadosAExcluir) {
			$this->db->where_in('id', array_column($indicadosAExcluir, 'id'))->delete('requisicoes_pessoal_candidatos');
			$this->db->where_in('id', array_column($indicadosAExcluir, 'id_usuario'))->delete('recrutamento_usuarios');
		}

		$postIndicados = [
			'id' => $rows['id_indicado'],
			'nome' => $rows['nome_indicado'],
			'nome_mae' => $rows['nome_mae_indicado'],
			'nome_pai' => $rows['nome_pai_indicado'],
			'data_nascimento' => array_map(function ($data_nascimento) {
				return date('Y-m-d', strtotime(str_replace('/', '-', $data_nascimento)));
			}, $rows['data_nascimento_indicado']),
			'rg' => $rows['rg_indicado'],
			'rg_data_emissao' => array_map(function ($rg_data_emissao) {
				return date('Y-m-d', strtotime(str_replace('/', '-', $rg_data_emissao)));
			}, $rows['rg_data_emissao_indicado']),
			'rg_orgao_emissor' => $rows['rg_orgao_emissor_indicado'],
			'cpf' => $rows['cpf_indicado'],
			'pis' => $rows['pis_indicado']
		];

		$dataGroup = [];

		foreach ($postIndicados as $indicado => $dadosIndicado) {
			foreach ($dadosIndicado as $dadoIndicado) {
				$dataGroup[$indicado][$dadoIndicado] = $dadoIndicado;
			}
		}

		$idNovosIndicados = [];

		foreach ($dataGroup as $k => $data) {
			if (strlen($data['nome']) == 0) {
				exit(json_encode(['erro' => 'O nome do Indicador ' . ($k + 1) . ' é obrigatório.']));
			}
			...
			$cpfEmUso = $this->db
				...
				->num_rows();
			if ($cpfEmUso) {
				exit(json_encode(['erro' => 'O CPF do Indicador ' . ($k + 1) . ' já está em uso.']));
			}

			if ($data['id']) {
				$this->db->update('recrutamento_usuarios', $data, ['id' => $data['id']]);
			} else {
				...
				$idNovosIndicados[] = $this->db->insert_id();
			}
		}

		if ($idNovosIndicados) {
			$dadosCandidato = [
				'id_requisicao' => $rows['id'],
				'status' => 'A',
				'aprovado' => 1,
				'aprovado_indicacao' => 1
			];

			foreach ($idNovosIndicados as $idNovoIndicado) {
				$dadosCandidato['id_usuario'] = $idNovoIndicado;
				$this->db->insert('requisicoes_pessoal_candidatos', $dadosCandidato);
			}
		}
	}
...
//            }
	...
	public function ajax_relatorio($pdf = false)
	{
		...
		$sql = "SELECT a.id, 
                       ...
                       DATE_FORMAT(a.data_suspensao, '%d/%m/%Y') AS data_suspensao,
                       DATE_FORMAT(a.data_cancelamento, '%d/%m/%Y') AS data_cancelamento,
                       DATE_FORMAT(a.data_processo_seletivo, '%d/%m/%Y') AS data_processo_seletivo,
		...
		$this->db->select("DATE_FORMAT(a.data_admissao, '%d/%m/%Y') AS data_admissao", false);
		...
		$row->candidatos_aprovados = $candidatosAprovados;
...
//        $this->db->select('perfil_profissional_desejado');
	...
	public function informarPendencia()
	{
		$data = $this->input->post();

		if (empty($data['id'])) {
			exit(json_encode(['erro' => 'Requisição de Pessoal não encontrada ou inexistente.']));
		}

		$selecionador = $this->db->select('id, nome, email')->where('id', $this->session->userdata('id'))->get('usuarios')->row();
		$requisitante = $this->db->select('id, nome, email')->where('id', $data['requisitante_interno'])->get('usuarios')->row();

		$this->load->library('email');

		$emailEnviado = $this->email
			->from($selecionador->email, $selecionador->nome)
			->to($requisitante->email)
			->subject("Pendência na Requisição de Pessoal  Nº {$data['id']}.")
			->message(nl2br($data['descricao_pendencias']))
			->send();
	...
	public function teste($modulo = null)
	{
		$id = $this->session->userdata('id');
		$empresa = $this->session->userdata('empresa');
		$tipo = $this->session->userdata('tipo');
		...
		$this->load->view('requisicaoPessoal__new', $data);
	}
