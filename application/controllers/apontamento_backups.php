...
class Apontamento_backups extends MY_Controller
{
    ...
    public function gerenciar($pdf = false)
    {
        ...
        if ($pdf) {
            ...
            $this->db->join('alocacao_usuarios b', 'a.id_alocado = b.id');
            ...
            $data['rows'] = $this->db->get('alocacao_apontamento a')->result();

            return $this->load->view('apontamento_backupsPdf', $data, true);
        } else {
            $this->load->view('apontamento_backups', $data);
        }
    }
    ...
    public function pdf()
    {
        ...
        $row = $this->db->get('alocacao_contratos a')->row();
