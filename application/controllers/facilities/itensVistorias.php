...
class ItensVistorias extends MY_Controller
{
    ...
    {
        $this->db->select('a.id AS idItem, a.nome AS nomeItem');
        ...
        $this->load->view('facilities/itens_vistorias', $data);
    }
    ...
    public function ajaxList()
    {
        ...
        foreach ($rows as $row) {
            $data[] = array(
                $row->nome,
                '<button class="btn btn-sm btn-info" onclick="edit_vistoria(' . $row->id . ')" title="Editar item de vistoria de facilities"><i class="glyphicon glyphicon-pencil"></i></button>
                 <button class="btn btn-sm btn-danger" onclick="delete_vistoria(' . $row->id . ')" title="Excluir item de vistoria facilities"><i class="glyphicon glyphicon-trash"></i></button>'
            );
        }
    ...
    public function ajaxEdit()
    {
        ...
        $this->db->where('b.ativo', 0);
        $data = $this->db->get('facilities_vistorias a')->row();
    ...
    public function ajaxUpdate()
    {
        ...
        $status = $this->db->update('facilities_vistorias', $data, array('id' => $id));
