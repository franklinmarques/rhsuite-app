...
class Ativos extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('facilities/ativos', $data);
    }
    ...
    public function ajaxList()
    {
        ...
        foreach ($rows as $row) {
            $data[] = array(
                ...
                $row->sala,
                $row->nome,
                '<button class="btn btn-sm btn-info" onclick="edit_ativo(' . $row->id . ')" title="Editar ativo"><i class="glyphicon glyphicon-pencil"></i></button>
                 <button class="btn btn-sm btn-danger" onclick="delete_ativo(' . $row->id . ')" title="Excluir ativo"><i class="glyphicon glyphicon-trash"></i></button>
    ...
    public function ajaxEdit()
    {
        ...
        $this->db->where('id', $id);
        $this->db->where('ativo', 1);
        $data = $this->db->get('facilities_itens')->row();
    ...
    public function ajaxAdd()
    {
        ...
        if (strlen($data['nome']) == 0) {
            exit(json_encode(['erro' => 'O nome do ativo é obrigatório']));
        }
        if ($data['data_entrada_operacao']) {
