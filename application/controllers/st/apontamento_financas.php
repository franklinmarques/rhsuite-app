...
class Apontamento_financas extends MY_Controller
{
    ...
    public function gerenciar($pdf = false)
    {
        ...
        $this->db->join('alocacao_contratos b', 'b.depto = a.depto AND b.area = a.area AND b.contrato = a.contrato');
        $this->db->join('alocacao_unidades c', 'c.id_contrato = b.id AND c.setor = a.setor');
        $this->db->join('alocacao_reajuste d', 'd.id_cliente = b.id AND d.data_reajuste <= a.data');
        ...
        if ($pdf) {
            ...
            return $this->load->view('apontamento_financasPdf', $data, true);
        } else {
    ...
    private function ajax_list()
    {
        ...
        $sqlReajuste = "SELECT d.data_reajuste AS data, d.id_cliente, d.valor_indice
                        FROM alocacao a 
                        INNER JOIN alocacao_contratos b 
                                   ON b.depto = a.depto
                                   AND b.area = a.area
                                   AND b.contrato = a.contrato
                        INNER JOIN alocacao_unidades c
                                   ON c.id_contrato = b.id
                                   AND c.setor = a.setor
                        INNER JOIN alocacao_reajuste d 
