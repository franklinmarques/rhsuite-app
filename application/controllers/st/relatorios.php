...
class Relatorios extends MY_Controller
{
    ...
    public function gerenciar($pdf = false)
    {
        ...
        if ($pdf) {
            return $this->load->view('st/relatorio', $data, true);
        } else {
            $this->load->view('st/relatorio', $data);
        }
    }
    ...
    private function ajax_list()
    {
        ...
        $sqlDias = "SELECT GROUP_CONCAT(CONCAT(' dia_', DATE_FORMAT(x.daynum, '%d'))) AS titulo,
                           GROUP_CONCAT(CONCAT(' (SELECT IF(\'', x.daynum, '\' <= CURDATE(), IF(e.id IS NOT NULL AND (e.status = \'FR\' OR e.data <= CURDATE()), CONCAT( GROUP_CONCAT(
                                                        ...
                                                        ELSE null END
                                                    )), null), null) FROM st_apontamento e
...
//        $sql .= ' GROUP BY a.id_usuario) s ORDER BY s.nome';
    ...
    public function ajax_servicos($id_contrato = null)
    {
        ...
        $this->db->select('contrato AS nome, descricao_servico, valor_servico');
        $this->db->where("DATE_FORMAT(data, '%Y-%m') <=", $ano . '-' . $mes);
        if ($depto) {
        ...
        $data = $this->db->get('st_contratos')->row();
        ...
        if ($data) {
            ...
            $this->db->where('a.data_reajuste', "(SELECT MAX(a2.data_reajuste) FROM st_servicos a2 WHERE DATE_FORMAT(a2.data_reajuste, '%Y-%m') <= '{$ano}-{$mes}')", false);
            $rows = $this->db->get('st_servicos a')->result();
...
//        $this->db->where('a.data_reajuste <', $dataAbertura);
    ...
    public function atividades_mensais($pdf = false)
    {
        ...
        $rows = $this->db->get('st_observacoes a')->result();
        ...
        $st_observacoes = $this->db->list_fields('st_observacoes');
        unset($st_observacoes['id'], $st_observacoes['id_alocacao']);
        $data = array_flip($st_observacoes);
        ...
        $this->db->where("DATE_FORMAT(b.data, '%Y') =", $get['ano']);
        $this->db->order_by('b.data', 'asc');
        $data['total_indicadores_operacionais'] = $this->db->get('st_observacoes a')->row();


        if ($pdf) {
            return $this->load->view('st/atividades_mensais', $data, true);
        } else {
            $this->load->view('st/atividades_mensais', $data);
        }
