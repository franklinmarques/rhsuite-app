...
class Apontamento_gestao_consolidada extends MY_Controller
{
    ...
    public function gerenciar($pdf = false)
    {
        ...
        if ($pdf) {
            ...
            return $this->load->view('apontamento_gestao_consolidadaPdf', $data, true);
        } else {
            $this->load->view('apontamento_gestao_consolidada', $data);
        }
    }
    ...
    private function ajax_list()
    {
        ...
        $sql = "SELECT t.setor,
                ...
                FROM (SELECT DISTINCT(s.setor) AS setor,
                             x.mes_ano,
                             x.mes_ano_completo,
                             x.data,
                             ...
                             (SELECT COUNT(DISTINCT(s1.id))
                              FROM usuarios s1
                              INNER JOIN alocacao_usuarios s2
                                         ON s2.id_usuario = s1.id
                              INNER JOIN alocacao s3 
                                         ON s3.id = s2.id_alocacao 
                                         AND s3.depto = s1.depto 
                                         AND s3.area = s1.area 
                                         AND s3.setor = s1.setor 
                              WHERE s1.tipo = 'funcionario'
                                    AND s1.depto = '{$busca['depto']}' 
                                    AND s1.area = '{$busca['area']}' 
                                    AND s1.status = 1 
                                    AND s3.data = x.data) AS usuarios_ativos,
                             COUNT(CASE WHEN (s.id IS NOT NULL AND s.status = 1) THEN s.id ELSE NULL END) AS usuarios_ativos2,
                             ...
                             (SELECT SUM(s1.qtde_alocados_potenciais)
                              FROM alocacao s1
                              WHERE s1.depto = '{$busca['depto']}' 
                                    AND s1.area = '{$busca['area']}' 
                                    AND s1.data = x.data) AS qtde_alocados_potenciais,
                             ...
                             SUM(s.qtde_faltas) + (SUM((s.horas_atraso * 60) + s.minutos_atraso) / 480) AS total_dias,
                             SUM((s.horas_atraso * 60) + s.minutos_atraso) / 480 AS horas_atraso,
                             SUM(s.qtde_faltas) AS qtde_faltas,
                             SUM(s.dias_ausentes) AS dias_ausentes,
                             IFNULL(SUM(s.posto_descoberto), 0) AS posto_descoberto,
                             
                             (SELECT SUM(p.valor_projetado) FROM alocacao p WHERE p.depto = s.depto AND p.area = s.area AND p.data = s.data) AS valor_projetado,
                             (SELECT SUM(p.valor_realizado) FROM alocacao p WHERE p.depto = s.depto AND p.area = s.area AND p.data = s.data) AS valor_realizado,
                             s.valor_projetado AS valor_projetado3,
                             SUM(s.valor_realizado) AS valor_realizado3,
                             IFNULL(SUM(s.valor_posto), 0) AS valor_projetado2,
                             IFNULL(SUM((s.valor_posto) - (IFNULL(s.qtde_dias, 0) * s.valor_dia + IFNULL(s.segundos_atraso / 3600, 0) * s.valor_hora) + IFNULL(s.total_acrescido, 0)), 0) AS valor_realizado2,
                             
                             (SELECT SUM(p.turnover_reposicao) FROM alocacao p WHERE p.depto = s.depto AND p.area = s.area AND p.data = s.data) AS turnover_reposicao,
                             (SELECT SUM(p.turnover_aumento_quadro) FROM alocacao p WHERE p.depto = s.depto AND p.area = s.area AND p.data = s.data) AS turnover_aumento_quadro,
                             (SELECT SUM(p.turnover_desligamento_empresa) FROM alocacao p WHERE p.depto = s.depto AND p.area = s.area AND p.data = s.data) AS turnover_desligamento_empresa,
                             (SELECT SUM(p.turnover_desligamento_colaborador) FROM alocacao p WHERE p.depto = s.depto AND p.area = s.area AND p.data = s.data) AS turnover_desligamento_colaborador,
                             SUM(s.turnover_reposicao) AS turnover_reposicao3,
                             SUM(s.turnover_aumento_quadro) AS turnover_aumento_quadro3,
                             SUM(s.turnover_desligamento_empresa) AS turnover_desligamento_empresa3,
                             SUM(s.turnover_desligamento_colaborador) AS turnover_desligamento_colaborador3                          
        ...
        /*$this->db->select('data, setor, SUM(valor_posto) AS valor_posto', false);
        $this->db->group_by('id_alocacao');
        $postos = $this->db->get('view_alocacao')->result();*/

        $sqlReajuste = "SELECT a.data_reajuste AS data, a.id_cliente, a.valor_indice
                        FROM alocacao_reajuste a
                        INNER JOIN alocacao_contratos b ON b.id = a.id_cliente
                        INNER JOIN alocacao_unidades c ON c.id_contrato = b.id
                        WHERE (b.depto = '{$busca['depto']}' OR CHAR_LENGTH('{$busca['depto']}') = 0) AND 
                              (b.area = '{$busca['area']}' OR CHAR_LENGTH('{$busca['area']}') = 0) 
        ...
        foreach ($rows as $row) {
            ...
            $row->perda_receita = ($reajuste_projetado - $reajuste_realizado) * 100 / max($reajuste_projetado, 1);
            $row->receita_liquida = $reajuste_realizado * 100 / max($reajuste_projetado, 1);
        }
