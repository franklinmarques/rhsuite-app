...
class Colaboradores extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('st/colaboradores', $data);
    }
...
//                      DATE_FORMAT(b.data, '%Y-%m') = '{$mes_ano}'";
        ...
        $this->load->view('st/getcolaboradores', $data);
    }
...
//            $funcionario->hash_acesso = json_decode($funcionario->hash_acesso, true);
        ...
        $this->load->view('st/coladorador', $data);
    }
    ...
    public function ajax_save()
    {
        ...
        $this->db->trans_start();
        $this->db->query($this->db->insert_string('st_alocados', $data));
        $this->db->trans_complete();
    ...
    public function ajax_save2()
    {
        ...
        $sql2 = "SELECT a.id, b.id_usuario
                 FROM alocacao a
                 LEFT JOIN st_alocados b ON b.id_alocacao = a.id AND b.id_usuario = '{$post['id_usuario']}'
        ...
        $this->db->trans_start();
        /*if ($count == 0) {
            $this->db->query($this->db->insert_string('st_postos', $data));
        }*/
        $this->db->query($this->db->insert_string('st_alocados', $data2));
        $this->db->trans_complete();
    ...
    {
        $id = $this->input->post('id');
        $status = $this->db->delete('st_alocados', array('id' => $id));
    ...
    public function eventos()
    {
        ...
        $this->load->view('st/eventos2', $data);
    }
