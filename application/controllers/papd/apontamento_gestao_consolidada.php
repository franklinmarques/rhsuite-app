    ...
    private function ajax_list()
    {
        ...
        $sql = "SELECT t.mes_ano, 
                ...
                FROM (SELECT x.mes_ano,
                             ...
                             COUNT(CASE WHEN (s.id IS NOT NULL AND s.status = 1) THEN s.id ELSE NULL END) AS usuarios_ativos,
                             ...
                             IFNULL(SUM(s.valor_posto), 0) AS valor_projetado,
                             IFNULL(SUM(s.valor_posto - (IFNULL(s.qtde_dias, 0) * s.valor_dia + IFNULL(s.segundos_atraso / 3600, 0) * s.valor_hora) + IFNULL(s.total_acrescido, 0)), 0) AS valor_realizado,
                       ...
                       /*SUM(s.posto_descoberto + s.dias_ausentes + s.qtde_faltas) + (SUM((s.horas_atraso * 60) + s.minutos_atraso) / 480) + SUM(s.qtde_dias_cobertos) AS total_faltas,
                       ...
                       FORMAT(IFNULL(SUM(s.valor_posto), 0), 2, 'de_DE') AS valor_projetado,
                       FORMAT(IFNULL(SUM(s.valor_posto - (IFNULL(s.qtde_dias, 0) * s.valor_dia + IFNULL(s.segundos_atraso / 3600, 0) * s.valor_hora) + IFNULL(s.total_acrescido, 0)), 0), 2, 'de_DE') AS valor_realizado,
        ...
        foreach($rows as $row){
            ...
            $row->perda_receita = ($reajuste_projetado - $reajuste_realizado) * 100 / $reajuste_projetado;
            $row->receita_liquida = $reajuste_realizado * 100 / $reajuste_projetado;
        }
