...
class Apontamento_totalizacao extends MY_Controller
{
...
//        $this->db->where('c.id', $id);
        ...
        $data['detalhes'] = $this->db->get('alocacao_apontamento a')->result();

        $this->load->view('apontamento_totalizacao', $data);
    }
...
//            }
    ...
    public function fecharMes()
    {
                ...
                FROM (SELECT x.data,
                             ...
                             CASE s.setor WHEN 'São José dos Campos' 
                                  THEN (SELECT SUM(IFNULL(s2.dias_faltas, 0) + IFNULL(s2.segundos_atraso / 28800, 0))
                                        ...
                                        INNER JOIN alocacao_usuarios s3 ON s3.id = s2.id
        ...
        /*$sqlIpesp = "SELECT SUM(b.valor) AS valor
                            ...
                            LEFT JOIN alocacao_servicos b ON b.id_contrato = a.id AND b.tipo = 1
                            LEFT JOIN alocacao_unidades c ON c.id_contrato = a.id
    ...
    public function ajax_edit()
    {
        ...
        $this->db->where('CHAR_LENGTH(a.detalhes) >', 0);
        $this->db->order_by('a.detalhes', 'asc');
        $detalhes = $this->db->get('alocacao_apontamento a')->result();
    ...
    public function ajax_save()
    {
        ...
        $status = $this->db->update('alocacao_usuarios', $data, array('id' => $id));
