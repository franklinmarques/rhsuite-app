...
class Pagina_curso extends MY_Controller
{
    ...
    public function ajax_list()
    {
        ...
        $config['base_url'] = site_url('ead/pagina_curso/ajax_list/' . $curso->id);
        $config['total_rows'] = $this->db->query("SELECT * FROM cursos_paginas {$qryWHERE}", $dataWHERE)->num_rows();
        $config['per_page'] = 20;
        ...
        $data['total'] = $this->db->query("SELECT * FROM cursos_paginas WHERE id_curso = ?", array($curso->id))->num_rows();
        $data['busca'] = "busca={$query}&{$this->security->get_csrf_token_name()}={$this->security->get_csrf_hash()}";
        $data['query'] = $this->db->query("SELECT * FROM cursos_paginas {$qryWHERE} ORDER BY ordem ASC LIMIT ?,?", $dataWHERE);

        $this->load->view('ead/getpaginascurso', $data);
    }
    ...
    public function ordenar()
    {
        ...
        $this->db->where_in('ordem', array_keys($rows));
        $this->db->update('cursos_paginas');

        foreach ($rows as $ordem => $id) {
            $this->db->update('cursos_paginas', array('ordem' => $ordem), array('id' => $id));
        }
    }
...
//                    }
    ...
    public function duplicar()
    {
        ...
        if (isset($data['id'])) {
            ...
            $this->db->query("UPDATE cursos_paginas SET ordem = ordem + 1 WHERE id_curso = {$data['id_curso']} AND ordem > {$data['ordem']}");
            $data['ordem'] += 1;
            ...
            $this->db->where('b.id', $novo_id);
            $rows = $this->db->get('cursos_questoes a')->result_array();

            if ($rows) {
                $this->db->insert_batch('cursos_questoes', $rows);
                ...
                $rows2 = $this->db->get('cursos_alternativas a')->result_array();
                if ($rows2) {
