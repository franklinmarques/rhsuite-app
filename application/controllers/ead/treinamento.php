...
class Treinamento extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data['categorias'] = $this->db->query("SELECT distinct(a.categoria) FROM cursos a INNER JOIN cursos_usuarios b ON b.id_curso = a.id WHERE b.id_usuario = {$usuario} AND CHAR_LENGTH(a.categoria) > 0");
        $data['areas_conhecimento'] = $this->db->query("SELECT distinct(a.area_conhecimento) FROM cursos a INNER JOIN cursos_usuarios b ON b.id_curso = a.id WHERE b.id_usuario = {$usuario} AND CHAR_LENGTH(a.area_conhecimento) > 0");
    ...
    public function ajax_list()
    {
        if ($this->session->userdata('tipo') == 'funcionario') {
            ...
            $view = 'ead/getmeuscursos';
            $redirect = site_url('home');
        } elseif ($this->session->userdata('tipo') == 'empresa') {
            $usuario = $this->uri->rsegment(3, 0);
            $limit = $this->uri->rsegment(4, 0);
            $view = 'ead/getcursosfuncionario';
        ...
        $ver_usuario = $this->db->get_where('usuarios', array('id' => $usuario))->num_rows();
        if ($ver_usuario == 0) {
            redirect($redirect);
        }
        ...
        $config['base_url'] = site_url('ead/treinamento/ajax_list');
    ...
    public function acessar()
    {
        $sql = "SELECT s.id, 
                ...
                FROM (SELECT a.id, 
                      ...
                      INNER JOIN cursos_paginas b ON 
                                 b.id_curso = a.id 
                      INNER JOIN cursos_usuarios c ON 
                                 c.id_curso = a.id 
                      LEFT JOIN cursos_acessos d ON 
                                d.id_curso_usuario = c.id AND 
                                d.id_pagina = b.id 
                      WHERE c.id = {$this->uri->rsegment(3, 0)} AND 
                            c.id_usuario = {$this->session->userdata('id')}) s";
        ...
        $this->db->join('cursos_usuarios b', 'b.id_curso = a.id_curso');
        $this->db->join('cursos_acessos c', 'c.id_curso_usuario = b.id AND c.id_pagina = a.id', 'left');
        $this->db->join('cursos_resultado d', 'd.id_acesso = c.id', 'left');
        $this->db->where('a.id_curso', $curso->id);
        $this->db->where('b.id_usuario', $this->session->userdata('id'));
        ...
        if ($this->input->server('SERVER_PORT') == 443 && preg_match('/<iframe>|<\/iframe>/i', $pagina_atual->conteudo)) {
            ...
            $qtdeHTTP = substr_count($pagina_atual->conteudo, 'http://');
...
//                header("Location: " . str_replace('https', 'http', current_url()));
        ...
        switch ($pagina_atual->modulo) {
            ...
            case 'atividades':
                ...
                $sql2 = "SELECT c.id_questao, c.id_alternativa, c.resposta 
                         FROM cursos_acessos a 
                         INNER JOIN cursos_paginas b ON 
                                    b.id = a.id_pagina
                         INNER JOIN cursos_resultado c ON 
                                    c.id_acesso = a.id
        ...
        if ($pagina_atual->acesso == 0) {
            ...
            $this->db->insert('cursos_acessos', $arrAcesso);
            ...
            $this->db->update('cursos_acessos', $arrAcesso, $where);
        }
        ...
        $this->load->view('ead/acessarcurso', $data);
    }
    ...
    public function atualizarTempoEstudo()
    {
        ...
        if ($row) {
            ...
            $status = $this->db->update('cursos_acessos');
        } else {
            $status = false;
        }
    ...
    public function avaliar_atividade()
    {
        ...
        $num_rows = $this->db->get('cursos_resultado')->num_rows();
        if ($num_rows) {
        ...
        $this->db->insert_batch('cursos_resultado', $data);
        ...
        if ($row->modulo == 'quiz' || $row->modulo == 'atividades') {
            $sql = "SELECT SUM(b.peso) AS total, 
                    ...
                    INNER JOIN cursos_alternativas b ON 
                               b.id_questao = a.id
                    LEFT JOIN cursos_resultado c ON 
    ...
    public function status($id = null)
    {
        $sql = "SELECT a.titulo,
                ...
                LEFT JOIN cursos_alternativas e ON 
                          e.id_questao = d.id
                LEFT JOIN cursos_acessos f ON 
                          f.id_curso_usuario = c.id AND 
                          f.id_pagina = a.id 
                LEFT JOIN cursos_resultado g ON 
    ...
    public function certificado($id)
    {
        $sql = "SELECT b.*, 
                       ...
                       b.horas_duracao,
                       h.nome AS nome_usuario, 
                       h.empresa, 
                       h.foto,
                       h.assinatura_digital, 
                       i.nome AS empresa_emissora, 
                       i.foto AS foto_empresa_emissora,
                       i.assinatura_digital AS assinatura_emissora,
                       a.data_inicio,
                       a.data_maxima,
                       a.nota_aprovacao,
                       e.peso,
