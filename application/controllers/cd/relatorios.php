...
//        $this->db->where("DATE_FORMAT(a.data, '%m/%Y') =", "{$data['mes']}/{$data['ano']}");
    ...
    private function ajax_list()
    {
        ...
        $sql .= "d.nome AS nome_cuidador
                 ...
                 INNER JOIN cd_diretorias c ON 
                            c.id = b.id_diretoria
                 INNER JOIN (SELECT @rownum:= @rownum + 1 AS rownum, a.* 
                             FROM (SELECT b.*
                                   FROM cd_cuidadores a
                 ...
                 LEFT JOIN cd_supervisores h ON
                           h.id_escola = b.id
        ...
        $sql .= ' GROUP BY a.id) s ORDER BY s.escola ASC';
    ...
    private function ajax_funcionarios()
    {
        ...
        for ($i = 1; $i <= 31; $i++) {
            $dia = str_pad($i, 2, '0', STR_PAD_LEFT);
            if (strtotime("{$busca['ano']}-{$busca['mes']}-$dia") <= strtotime(date('Y-m-d'))) {
                $sql .= "(SELECT CASE WHEN (h.status = 'FE' OR h.data <= CURDATE())
        ...
        $sql .= " CASE c.turno WHEN 'M' THEN 1
                               WHEN 'T' THEN 2
                               WHEN 'N' THEN 3 
                      ...
                      INNER JOIN cd_alocacao b ON 
                                 b.id = a.id_alocacao
                      INNER JOIN cd_cuidadores c ON 
                      c.id = a.id_vinculado
                      INNER JOIN usuarios d ON
                      d.id = c.id_cuidador
                      INNER JOIN cd_escolas e ON 
                      e.id = c.id_escola
                      INNER JOIN cd_diretorias f ON 
                                 f.id = e.id_diretoria
                      LEFT JOIN cd_supervisores g ON
                                g.id_escola = e.id
                      WHERE f.id_empresa = {$this->session->userdata('empresa')} AND 
                            DATE_FORMAT(b.data, '%Y-%m') = '{$busca['ano']}-{$busca['mes']}'";
        if (!empty($busca['depto'])) {
            $sql .= " AND f.depto = '{$busca['depto']}'";
        }
        if (!empty($busca['diretoria'])) {
            $sql .= " AND f.id = '{$busca['diretoria']}'";
        }
        if (!empty($busca['supervisor'])) {
            $sql .= " AND g.id_supervisor = '{$busca['supervisor']}'";
        }
        $sql .= ') s';
        $data = $this->db->query($sql)->result();
    ...
    private function ajax_insumos()
    {
        ...
        $sql = "SELECT d.id AS id_escola, 
                       a.escola, 
                       (SELECT COUNT(DISTINCT(s.id_aluno)) 
                        FROM cd_matriculados s 
                        WHERE s.escola = d.nome AND 
                              s.id_alocacao = b.id) AS total_alunos,
                       a.aluno AS aluno,
                       {$colunas}, 
                       SUM(g.qtde) AS total
                FROM cd_matriculados a 
                INNER JOIN cd_alocacao b ON
                           b.id = a.id_alocacao
                INNER JOIN cd_diretorias c ON
                ...
                GROUP BY a.escola, a.aluno
                ORDER BY a.escola, a.aluno";
        ...
        $sqlNomeColunas = "SELECT LCASE(REPLACE(REPLACE(a.nome, '-', ''), ' ', '_')) AS id, 
                           ...
                           LEFT JOIN (SELECT b.id_insumo, 
                                             b.qtde 
                                      FROM cd_consumos b 
                                      INNER JOIN cd_frequencias c 
                                                 ON c.id = b.id_frequencia 
                                                 AND DATE_FORMAT(c.data, '%Y-%m') = '{$ano}-{$mes}' 
                                      INNER JOIN cd_matriculados d 
                                                 ON d.id = c.id_matriculado
                                      INNER JOIN cd_alocacao e 
                                      ...
                                      INNER JOIN cd_supervisores h
                                                 ON h.id_escola = g.id
                                                 AND h.turno = d.turno
                                                 AND (h.id_supervisor = '{$supervisor}' OR CHAR_LENGTH('{$supervisor}') = 0)) s 
                                     ON s.id_insumo = a.id
        ...
        $sql = "SELECT s.id,
                ...
                FROM (SELECT a.id,
                             ...
                             (CASE b.turno 
                                   ...
                                   WHEN 'N' THEN 'Noite'
                                   END) AS turno,
				             GROUP_CONCAT(CONCAT('<strong>• </strong>', b.cuidador) ORDER BY b.cuidador SEPARATOR '<br>') AS cuidador,
                             DATE_FORMAT(c.data_admissao, '%d/%m/%Y') AS data_admissao,
                             GROUP_CONCAT(CONCAT('<strong>• </strong>',
                             IF(CHAR_LENGTH(c.valor_vt) > 0,CONCAT(c.nome_cartao, ' (', c.valor_vt, ')'), TRIM(c.nome_cartao)))
                              ORDER BY b.cuidador SEPARATOR '<br>') AS vale_transporte,
                             d.aluno,
                             d.hipotese_diagnostica
                      FROM cd_alocacao a
                      INNER JOIN cd_alocados b 
                                 ON b.id_alocacao = a.id
                      INNER JOIN (SELECT c1.*, c2.id AS id_vinculado 
                                  FROM usuarios c1 
    ...
    public function pdfResultados()
    {
        ...
        $this->load->library('Calendar');
        $this->calendar->month_type = 'short';
        $nome = 'Cuidadores - Acompanhamento mensal ' . $this->calendar->get_month_name($data['mes']) . '_' . $data['ano'];
