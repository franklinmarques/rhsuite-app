...
class Cuidadores extends MY_Controller
{
	...
	public function index()
	{
		...
		$this->db->join('cd_supervisores d', 'd.id_escola = b.id', 'left');
		...
		$busca_deptos = $this->db->get('cd_cuidadores a')->result();
		...
		$this->db->order_by('c.depto', 'asc');
		$busca_diretorias = $this->db->get('cd_cuidadores a')->result();
		...
		$busca_escolas = $this->db->get('cd_cuidadores a')->result();
	...
	public function atualizar_filtro()
	{
		...
		$diretorias = $this->db->get('cd_cuidadores a')->result();
		foreach ($diretorias as $diretoria) {
		...
		$this->db->order_by('b.nome', 'asc');
		$escolas = $this->db->get('cd_cuidadores a')->result();
	...
	public function atualizar_unidades()
	{
		...
		$this->db->join('cd_cuidadores c', 'c.id_escola = b.id', 'left');
		$this->db->join('cd_supervisores d', 'd.id_escola = b.id', 'left');
		...
		$this->db->order_by('a.nome', 'asc');
		$rows = $this->db->get('cd_diretorias a')->result();
		...
		$this->db->select('a.id, a.nome');
		$this->db->join('cd_diretorias b', 'b.id = a.id_diretoria');
		$this->db->join('cd_cuidadores c', 'c.id_escola = a.id', 'left');
		$this->db->join('cd_supervisores d', 'd.id_escola = a.id', 'left');
		$this->db->where('b.id_empresa', $empresa);
		$this->db->where('c.id', null);
		if (in_array($this->session->userdata('nivel'), array(4, 10, 11))) {
			$this->db->where('d.id_supervisor', $id_usuario);
		...
		$this->db->order_by('a.nome', 'asc');
		$rows2 = $this->db->get('cd_escolas a')->result();
	...
	public function atualizar_cuidadores()
	{
		...
		$this->db->order_by('a.nome', 'asc');
		$rows = $this->db->get('cd_escolas a')->result();
		$escolas = array('' => 'selecione...');
		$depto = $rows[0]->depto ?? '';
		foreach ($rows as $row) {
		...
		$this->db->select('a.id, a.nome');
		$this->db->join('cd_diretorias b', 'b.alias = a.area');
		$this->db->join('cd_escolas c', 'c.id_diretoria = b.id');
		$this->db->join('cd_cuidadores d', "d.id_cuidador = a.id AND d.turno = 'M'", 'left');
		...
		$this->db->join('cd_diretorias b', 'b.alias = a.area');
		$this->db->join('cd_escolas c', 'c.id_diretoria = b.id');
		$this->db->join('cd_cuidadores d', "d.id_cuidador = a.id AND d.turno = 'T'", 'left');
		$this->db->where('a.empresa', $empresa);
		$this->db->where('b.id', $id_diretoria);
		$this->db->where('c.id', $id_escola);
		$this->db->where('d.id', null);
		...
		$this->db->join('cd_diretorias b', 'b.alias = a.area');
		$this->db->join('cd_escolas c', 'c.id_diretoria = b.id');
		$this->db->join('cd_cuidadores d', "d.id_cuidador = a.id AND d.turno = 'N'", 'left');
		$this->db->where('a.empresa', $empresa);
		$this->db->where('b.id', $id_diretoria);
		$this->db->where('c.id', $id_escola);
		$this->db->where('d.id', null);
	...
	public function ajax_list()
	{
		...
		$sql = "SELECT s.id,
                ...
                FROM (SELECT a.id,
                      ...
                      INNER JOIN cd_diretorias d ON 
                                 d.id = c.id_diretoria
                      LEFT JOIN cd_supervisores e ON e.id_escola = c.id AND e.turno = a.turno
		...
		foreach ($list as $cd) {
			...
			$row[] = $cd->nome;
			$row[] = '
                      <button type="button" class="btn btn-sm btn-info" onclick="edit_cuidadores(' . $cd->id_escola . ')" title="Vincular cuidadores"><i class="glyphicon glyphicon-plus"></i> Vincular cuidadores</button>&emsp;&emsp;
                      <button type="button" class="btn btn-sm btn-danger" onclick="delete_cuidadores(' . $cd->id_escola . ')" title="Desvincular cuidadores"><i class="glyphicon glyphicon-minus"></i> </button>
                    ';
			$data[] = $row;
		}
	...
	public function ajax_edit()
	{
		...
		$row = $this->db->get('cd_escolas a')->row();
		...
		$this->db->join('cd_diretorias b', 'b.alias = a.area');
		$this->db->join('cd_escolas c', 'c.id_diretoria = b.id');
		$this->db->join('cd_cuidadores d', "d.id_cuidador = a.id AND d.id_escola = c.id AND d.turno = 'M'", 'left');
		$this->db->where('b.id', $id_diretoria);
		$this->db->where('c.id', $id_escola);
		$this->db->group_by('a.id');
		$this->db->order_by('a.nome', 'asc');
		...
		$this->db->join('cd_diretorias b', 'b.alias = a.area');
		$this->db->join('cd_escolas c', 'c.id_diretoria = b.id');
		$this->db->join('cd_cuidadores d', "d.id_cuidador = a.id AND d.id_escola = c.id AND d.turno = 'T'", 'left');
		$this->db->where('b.id', $id_diretoria);
		$this->db->where('c.id', $id_escola);
		$this->db->group_by('a.id');
		$this->db->order_by('a.nome', 'asc');
		...
		$this->db->join('cd_diretorias b', 'b.alias = a.area');
		$this->db->join('cd_escolas c', 'c.id_diretoria = b.id');
		$this->db->join('cd_cuidadores d', "d.id_cuidador = a.id AND d.id_escola = c.id AND d.turno = 'N'", 'left');
		$this->db->where('b.id', $id_diretoria);
		$this->db->where('c.id', $id_escola);
		$this->db->group_by('a.id');
		$this->db->order_by('a.nome', 'asc');
	...
	public function ajax_save()
	{
		...
		$delete = "DELETE s.* FROM cd_cuidadores s
                   LEFT JOIN (SELECT *
                              FROM cd_cuidadores 
		...
		$this->db->join('cd_cuidadores b', "b.id_cuidador = a.id AND b.id_escola = {$id_escola} AND b.turno = 'M'", 'left');
		$this->db->where_in('a.id', $id_manha);
		$this->db->where('b.id', null);
		$data_manha = $this->db->get('usuarios a')->result_array();
		if ($data_manha) {
			$this->db->insert_batch('cd_cuidadores', $data_manha);
		}
		...
		$this->db->join('cd_cuidadores b', "b.id_cuidador = a.id AND b.id_escola = {$id_escola} AND b.turno = 'T'", 'left');
		$this->db->where_in('a.id', $id_tarde);
		$this->db->where('b.id', null);
		$data_tarde = $this->db->get('usuarios a')->result_array();
		if ($data_tarde) {
			$this->db->insert_batch('cd_cuidadores', $data_tarde);
		}
		...
		$this->db->join('cd_cuidadores b', "b.id_cuidador = a.id AND b.id_escola = {$id_escola} AND b.turno = 'N'", 'left');
		$this->db->where_in('a.id', $id_noite);
		$this->db->where('b.id', null);
		$data_noite = $this->db->get('usuarios a')->result_array();
		if ($data_noite) {
			$this->db->insert_batch('cd_cuidadores', $data_noite);
		}
	...
	public function ajax_delete()
	{
		...
		$this->db->delete('cd_cuidadores', array('id_escola' => $id_escola));
