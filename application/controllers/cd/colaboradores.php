...
class Colaboradores extends MY_Controller
{
    ...
    public function index2()
    {
        if ($this->session->userdata('tipo') == 'funcionario') {
            ...
            $this->db->like('depto', 'cuidadores');
            $this->db->like('area', 'supervisão');
            $this->db->or_like('area', 'cuidadores');
            $this->db->limit(1);
            $filtro = $this->db->get('usuarios')->row();
            $data = $this->get_filtros_usuarios($filtro->depto);
        }
        ...
        if ($this->session->userdata('tipo') == 'funcionario') {
            ...
            $this->db->select("depto, area, '' AS setor", false);
            $this->db->like('depto', 'cuidadores');
            $this->db->like('area', 'supervisão');
            $this->db->or_like('area', 'cuidadores');
        }
...
//        $this->db->join('(SELECT @rownum:=0) b', 'a.id = a.id');
    ...
    public function getcolaboradores()
    {
        ...
        $config['base_url'] = site_url('cd/colaboradores/getcolaboradores');
...
//        $this->db->where_in('tipo', array('funcionario', 'selecionador'));
        ...
        if (count($funcionario) == 0) {
            redirect(site_url('cd/colaboradores'));
        }
...
//            $funcionario->hash_acesso = json_decode($funcionario->hash_acesso, true);
    ...
    public function save_perfil()
    {
        ...
        if ($this->db->where('id', $funcionario->id)->update('usuarios', $data)) {
            echo json_encode(array('retorno' => 1, 'aviso' => 'Funcionário editado com sucesso', 'redireciona' => 1, 'pagina' => site_url('cd/colaboradores')));
        } else {
