            ...
            <div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <div class="btn-group" role="group" aria-label="...">
                                        <button type="submit" id="limpa_filtro" class="btn btn-sm btn-default">Limpar filtros</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="row">
                    <div class="col col-xs-12">
                        <p class="text-right">
                            <button type="button" id="gerar_comparativo" onclick="gerar_comparativo()" class="btn btn-primary">Gerar comparativo</button>
                            <a class="btn btn-default" onclick="javascript:history.back()"><i class="glyphicon glyphicon-circle-arrow-left"></i> Voltar</a>
                        </p>
                    </div>
                </div>
        ...
        <table class="table table-condensed avaliacao">
            ...
            <tbody>
                <tr style='border-top: 5px solid #ddd;'>
                    ...
                    <td class="text-right">
                        <?php if ($is_pdf == false): ?>
                            <a id="pdf" class="btn btn-sm btn-danger" href="<?= site_url('competencias/relatorios/pdfAnalise_comparativa/' . $this->uri->rsegment(3)); ?>" title="Exportar PDF"><i class="glyphicon glyphicon-download-alt"></i> Exportar PDF</a>
                        <?php endif; ?>
                    </td>
                </tr>
            </tbody>
        ...
        <?php foreach ($cargos as $competencias): ?>
            <?php foreach ($competencias as $competencia): ?>
                <section class="panel panel-default">
                    <div class="panel-heading no-border" style="color: #333 !important; background-color: #f5f5f5 !important;"><strong><?= $competencia->nome ?></strong></div>
                    <!-- Table -->
                    <div class="panel-body" style="padding: 10px 15px;">
                        ...
                        <?php foreach ($competencia->dimensao as $k => $dimensao): ?>      
                            <table id="table_<?= $dimensao->id ?>" class="competencias table table-condensed">
                                <thead>
                                    <tr class='success text-success' style="border-top: 1px solid #ddd; border-bottom: 1px solid #ddd;">
                                            ...
                                            <tr class='text-right'>
                                                ...
                                                <?php switch (true): case $colaborador->atitude === null: ?>
                                                    ...
                                                    <?php case $colaborador->atitude > $dimensao->atitude: ?>
                                                        ...
                                                        <td class="text-success"><strong><?= $colaborador->gapAtitude ?></strong></td>
                                                        <?php break; ?>
                                                    <?php case $colaborador->atitude < $dimensao->atitude: ?>
                                                        <td class="text-danger"><strong><?= $colaborador->atitude ?></strong></td>
                                                        <td class="text-danger"><strong><?= $colaborador->gapAtitude ?></strong></td>
                                                        <?php break; ?>
                                                    <?php default: ?>
...
//            }
    ...
    function gerar_comparativo() {
        ...
        $.ajax({
            url: "<?php echo site_url('competencias/relatorios/ajax_analiseComparativa/') ?>/",
            type: "POST",
            dataType: "JSON",
            data: form,
            success: function (data)
