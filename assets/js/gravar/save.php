...
function selfInvoker()
{
    ...
    if (false && isset($_POST['audio-filename']) && strrpos($_POST['audio-filename'], "RecordRTC-") !== 0) {
        echo 'O nome do arquivo deve começar com "RecordRTC-"';
        return;
    }
    ...
    if (empty($fileName) || empty($tempName)) {
        if(empty($tempName)) {
            echo 'Inválido temp_name: '.$tempName;
            return;
        }

        echo 'Nome de arquivo inválido: '.$fileName;
        return;
    }
    ...
    */
    ...
    if (!$extension || empty($extension) || !in_array($extension, $allowed)) {
        echo 'Extensão de arquivo inválida: '.$extension;
        return;
    }
    ...
    if (!move_uploaded_file($tempName, $filePath)) {
        if(!empty($_FILES["file"]["error"])) {
            ...
            if(!empty($listOfErrors[$error])) {
                ...
                echo 'Não foi enviado por causa de erro #'.$_FILES["file"]["error"];
            }
        }
        else {
            echo 'Problema ao salvar o arquivo: '.$tempName;
        }
        return;
    }
